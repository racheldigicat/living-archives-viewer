import { $t as Plane, An as Scene, B as DepthFormat, Bn as SphereGeometry, Cn as Ray$1, E as Color, En as RedIntegerFormat, F as CylinderGeometry, Ft as MeshBasicMaterial, I as DataTexture, Jt as Object3D, Mt as Matrix3, Nt as Matrix4, Pt as Mesh, Qn as UnsignedIntType, Qt as PerspectiveCamera, R as DataUtils, U as DynamicDrawUsage, Un as TOUCH, V as DepthTexture, Vn as Spherical, Xt as OrthographicCamera, Z as FloatType, Zn as UnsignedByteType, _ as BufferAttribute, a as WebGLUtils, ar as WebGLRenderTarget, at as InstancedBufferGeometry, en as PlaneGeometry, er as Vector2, et as Group, g as BoxGeometry, h as Box3, i as WebGLRenderer, it as InstancedBufferAttribute, jn as ShaderMaterial, jt as MathUtils, k as ConeGeometry, kt as MOUSE, ln as RGBAFormat, nr as Vector4, on as Quaternion, q as EventDispatcher, tr as Vector3, tt as HalfFloatType, un as RGBAIntegerFormat, v as BufferGeometry } from "./three.module-Da2i9h0F.js";
//#region node_modules/@mkkellogg/gaussian-splats-3d/build/gaussian-splats-3d.module.js
/**
...
*/
var AbortablePromise = class AbortablePromise {
	static idGen = 0;
	...
	then(onResolve) {
		return new AbortablePromise((resolve, reject) => {
			this.promise = this.promise.then((...args) => {
				...
				if (onResolveResult instanceof Promise || onResolveResult instanceof AbortablePromise) onResolveResult.then((...args2) => {
					resolve(...args2);
				});
				else resolve(onResolveResult);
			}).catch((error) => {
				reject(error);
			});
		}, this.abortHandler);
...
(function() {
	const floatView = /* @__PURE__ */ new Float32Array(1);
...
var fetchWithProgress = function(path, onProgress, saveChunks = true, headers) {
	...
	const localOnProgress = (percent, percentLabel, chunk, fileSize) => {
		if (onProgress && !onProgressCalledAtComplete) {
			...
			if (percent === 100) onProgressCalledAtComplete = true;
		}
	};
	return new AbortablePromise((resolve, reject) => {
		const fetchOptions = { signal };
		if (headers) fetchOptions.headers = headers;
		fetch(path, fetchOptions).then(async (data) => {
			if (!data.ok) {
				const errorText = await data.text();
				reject(/* @__PURE__ */ new Error(`Fetch failed: ${data.status} ${data.statusText} ${errorText}`));
				return;
			}
			...
			while (!aborted) try {
				...
				if (done) {
					...
					if (saveChunks) resolve(new Blob(chunks).arrayBuffer());
					else resolve();
					break;
				}
...
var disposeAllMeshes = (object3D) => {
	...
	if (object3D.children) for (let child of object3D.children) disposeAllMeshes(child);
};
var delayedExecute = (func, fast) => {
	return new Promise((resolve) => {
		window.setTimeout(() => {
			resolve(func ? func() : void 0);
		}, fast ? 1 : 50);
	});
};
var getSphericalHarmonicsComponentCountForDegree = (sphericalHarmonicsDegree = 0) => {
	let shCoeffPerSplat = 0;
	if (sphericalHarmonicsDegree === 1) shCoeffPerSplat = 9;
	else if (sphericalHarmonicsDegree === 2) shCoeffPerSplat = 24;
	else if (sphericalHarmonicsDegree === 3) shCoeffPerSplat = 45;
	else if (sphericalHarmonicsDegree > 3) throw new Error("getSphericalHarmonicsComponentCountForDegree() -> Invalid spherical harmonics degree");
...
var abortablePromiseWithExtractedComponents = (abortHandler) => {
	...
	return {
		"promise": new AbortablePromise((resolve, reject) => {
			resolver = resolve;
			rejecter = reject;
		}, abortHandler),
		"resolve": resolver,
...
var UncompressedSplatArray = class UncompressedSplatArray {
	static OFFSET = {
		X: 0,
		Y: 1,
		Z: 2,
	...
	addSplatFromComonents(x, y, z, scale0, scale1, scale2, rot0, rot1, rot2, rot3, r, g, b, opacity, ...rest) {
		...
		for (let i = 0; i < rest.length && i < this.sphericalHarmonicsCount; i++) newSplat[i] = rest[i];
		this.addSplat(newSplat);
		return newSplat;
	}
	addSplatFromArray(src, srcIndex) {
		...
		const newSplat = UncompressedSplatArray.createSplat(this.sphericalHarmonicsDegree);
		for (let i = 0; i < this.componentCount && i < srcSplat.length; i++) newSplat[i] = srcSplat[i];
		this.addSplat(newSplat);
	}
};
var Constants = class {
...
var DefaultSphericalHarmonics8BitCompressionHalfRange = Constants.SphericalHarmonics8BitCompressionRange / 2;
...
var toUncompressedFloat = (f, compressionLevel, isSH = false, range8BitMin, range8BitMax) => {
	if (compressionLevel === 0) return f;
	else if (compressionLevel === 1 || compressionLevel === 2 && !isSH) return DataUtils.fromHalfFloat(f);
	else if (compressionLevel === 2) return fromUint8(f, range8BitMin, range8BitMax);
};
...
var dataViewFloatForCompressionLevel = (dataView, floatIndex, compressionLevel, isSH = false) => {
	if (compressionLevel === 0) return dataView.getFloat32(floatIndex * 4, true);
	else if (compressionLevel === 1 || compressionLevel === 2 && !isSH) return dataView.getUint16(floatIndex * 2, true);
	else return dataView.getUint8(floatIndex, true);
};
var convertBetweenCompressionLevels = function() {
	const noop = (v) => v;
	return function(val, fromLevel, toLevel, isSH = false) {
		...
		if (fromLevel === 2 && isSH) {
			...
			else if (toLevel == 0) outputConversionFunc = fromUint8;
...
var SplatBuffer = class SplatBuffer {
	...
	getMinSphericalHarmonicsDegree() {
		...
		for (let i = 0; i < this.sections.length; i++) {
			...
			if (i === 0 || section.sphericalHarmonicsDegree < minSphericalHarmonicsDegree) minSphericalHarmonicsDegree = section.sphericalHarmonicsDegree;
		}
	...
	getBucketIndex(section, localSplatIndex) {
		...
		if (localSplatIndex < maxSplatIndexInFullBuckets) bucketIndex = Math.floor(localSplatIndex / section.bucketSize);
		else {
			...
			while (bucketSplatIndex < section.splatCount) {
				...
				if (localSplatIndex >= bucketSplatIndex && localSplatIndex < bucketSplatIndex + currentPartiallyFilledBucketSize) break;
	...
	getSplatCenter(globalSplatIndex, outCenter, transform) {
		...
		if (this.compressionLevel >= 1) {
			const bucketBase = this.getBucketIndex(section, localSplatIndex) * SplatBuffer.BucketStorageSizeFloats;
	...
	getSplatColor(globalSplatIndex, outColor) {
		...
		const section = this.sections[sectionIndex];
		const localSplatIndex = globalSplatIndex - section.splatCountOffset;
		const srcSplatColorsBase = section.bytesPerSplat * localSplatIndex + SplatBuffer.CompressionLevels[this.compressionLevel].ColorOffsetBytes;
	...
	fillSplatCenterArray(outCenterArray, transform, srcFrom, srcTo, destFrom) {
		...
		for (let i = srcFrom; i <= srcTo; i++) {
			...
			if (transform) center.applyMatrix4(transform);
	...
	fillSplatScaleRotationArray = function() {
		...
		return function(outScaleArray, outRotationArray, transform, srcFrom, srcTo, destFrom, desiredOutputCompressionLevel, scaleOverride) {
			...
			for (let i = srcFrom; i <= srcTo; i++) {
				...
				scale.set(toUncompressedFloat(srcScaleX, this.compressionLevel), toUncompressedFloat(srcScaleY, this.compressionLevel), toUncompressedFloat(srcScaleZ, this.compressionLevel));
				rotation.set(toUncompressedFloat(srcRotationX, this.compressionLevel), toUncompressedFloat(srcRotationY, this.compressionLevel), toUncompressedFloat(srcRotationZ, this.compressionLevel), toUncompressedFloat(srcRotationW, this.compressionLevel)).normalize();
				if (transform) {
	...
	fillSphericalHarmonicsArray = function() {
		...
		for (let i = 0; i < 15; i++) sphericalHarmonicVectors[i] = new Vector3();
		...
		return function(outSphericalHarmonicsArray, outSphericalHarmonicsDegree, transform, srcFrom, srcTo, destFrom, desiredOutputCompressionLevel) {
			...
			for (let i = srcFrom; i <= srcTo; i++) {
				...
				const localSplatIndex = i - section.splatCountOffset;
				const srcSplatSHBase = section.bytesPerSplat * localSplatIndex + SplatBuffer.CompressionLevels[this.compressionLevel].SphericalHarmonicsOffsetBytes;
				...
				if (outSphericalHarmonicsDegree >= 1) {
					...
					if (outSphericalHarmonicsDegree >= 2) {
						...
						if (transform) {
							...
							SplatBuffer.rotateSphericalHarmonics5(shIn1, shIn2, shIn3, shIn4, shIn5, sh11, sh12, sh13, sh21, sh22, sh23, sh24, sh25, shOut1, shOut2, shOut3, shOut4, shOut5);
						} else {
							copy3(shIn1, shOut1);
							copy3(shIn2, shOut2);
							copy3(shIn3, shOut3);
	...
	static rotateSphericalHarmonics5 = (in1, in2, in3, in4, in5, tsh11, tsh12, tsh13, tsh21, tsh22, tsh23, tsh24, tsh25, out1, out2, out3, out4, out5) => {
	...
	static parseHeader(buffer) {
		...
		return {
			versionMajor: headerArrayUint8[0],
			versionMinor: headerArrayUint8[1],
			maxSectionCount: headerArrayUint32[1],
			sectionCount: headerArrayUint32[2],
			maxSplatCount: headerArrayUint32[3],
			splatCount: headerArrayUint32[4],
			compressionLevel: headerArrayUint16[10],
			sceneCenter: new Vector3(headerArrayFloat32[6], headerArrayFloat32[7], headerArrayFloat32[8]),
			minSphericalHarmonicsCoeff: headerArrayFloat32[9] || -DefaultSphericalHarmonics8BitCompressionHalfRange,
			maxSphericalHarmonicsCoeff: headerArrayFloat32[10] || DefaultSphericalHarmonics8BitCompressionHalfRange
		};
	}
	...
	static parseSectionHeaders(header, buffer, offset = 0, secLoadedCountsToMax) {
		...
		for (let i = 0; i < maxSectionCount; i++) {
			...
			const compressionScaleRange = sectionHeaderArrayUint32[sectionHeaderBaseUint32 + 6] || SplatBuffer.CompressionLevels[compressionLevel].ScaleRange;
	...
	static calculateComponentStorage(compressionLevel, sphericalHarmonicsDegree) {
		...
		const sphericalHarmonicsBytesPerSplat = SplatBuffer.CompressionLevels[compressionLevel].BytesPerSphericalHarmonicsComponent * sphericalHarmonicsComponentsPerSplat;
		return {
			bytesPerCenter,
			bytesPerScale,
			bytesPerRotation,
			...
			bytesPerSplat: bytesPerCenter + bytesPerScale + bytesPerRotation + bytesPerColor + sphericalHarmonicsBytesPerSplat
		};
	}
	linkBufferArrays() {
		for (let i = 0; i < this.maxSectionCount; i++) {
			const section = this.sections[i];
			section.bucketArray = new Float32Array(this.bufferData, section.bucketsBase, section.bucketCount * SplatBuffer.BucketStorageSizeFloats);
			if (section.partiallyFilledBucketCount > 0) section.partiallyFilledBucketLengths = new Uint32Array(this.bufferData, section.base, section.partiallyFilledBucketCount);
		}
	}
	buildMaps() {
	...
	static writeSplatDataToSectionBuffer = function() {
		const tempCenterBuffer = /* @__PURE__ */ new ArrayBuffer(12);
		const tempScaleBuffer = /* @__PURE__ */ new ArrayBuffer(12);
		const tempRotationBuffer = /* @__PURE__ */ new ArrayBuffer(16);
		const tempColorBuffer = /* @__PURE__ */ new ArrayBuffer(4);
		const tempSHBuffer = /* @__PURE__ */ new ArrayBuffer(256);
		const tempRot = new Quaternion();
		const tempScale = new Vector3();
		const bucketCenterDelta = new Vector3();
		const { X: OFFSET_X, Y: OFFSET_Y, Z: OFFSET_Z, SCALE0: OFFSET_SCALE0, SCALE1: OFFSET_SCALE1, SCALE2: OFFSET_SCALE2, ROTATION0: OFFSET_ROT0, ROTATION1: OFFSET_ROT1, ROTATION2: OFFSET_ROT2, ROTATION3: OFFSET_ROT3, FDC0: OFFSET_FDC0, FDC1: OFFSET_FDC1, FDC2: OFFSET_FDC2, OPACITY: OFFSET_OPACITY, FRC0: OFFSET_FRC0, FRC9: OFFSET_FRC9 } = UncompressedSplatArray.OFFSET;
		...
		return function(targetSplat, sectionBuffer, bufferOffset, compressionLevel, sphericalHarmonicsDegree, bucketCenter, compressionScaleFactor, compressionScaleRange, minSphericalHarmonicsCoeff = -DefaultSphericalHarmonics8BitCompressionHalfRange, maxSphericalHarmonicsCoeff = DefaultSphericalHarmonics8BitCompressionHalfRange) {
			...
			if (targetSplat[OFFSET_ROT0] !== void 0) {
				...
				tempRot.normalize();
			} else tempRot.set(1, 0, 0, 0);
			if (targetSplat[OFFSET_SCALE0] !== void 0) tempScale.set(targetSplat[OFFSET_SCALE0] || 0, targetSplat[OFFSET_SCALE1] || 0, targetSplat[OFFSET_SCALE2] || 0);
			else tempScale.set(0, 0, 0);
			if (compressionLevel === 0) {
				...
				if (sphericalHarmonicsDegree > 0) {
					...
					if (sphericalHarmonicsDegree >= 1) {
						...
						if (sphericalHarmonicsDegree >= 2) for (let s = 0; s < 15; s++) shOut[s + 9] = targetSplat[OFFSET_FRC9 + s] || 0;
					}
				}
			} else {
				...
				if (sphericalHarmonicsDegree > 0) {
					...
					if (sphericalHarmonicsDegree >= 1) {
						...
						if (sphericalHarmonicsDegree >= 2) {
							...
							copyBetweenBuffers(shOut.buffer, degree1ByteCount, sectionBuffer, sphericalHarmonicsBase + degree1ByteCount, 15 * bytesPerSHComponent);
						}
					}
				}
	...
	static generateFromUncompressedSplatArrays(splatArrays, minimumAlpha, compressionLevel, sceneCenter, blockSize, bucketSize, options = []) {
		...
		for (let sa = 0; sa < splatArrays.length; sa++) {
			...
			for (let i = 0; i < splatArray.splats.length; i++) {
				...
				for (let sc = UncompressedSplatArray.OFFSET.FRC0; sc < UncompressedSplatArray.OFFSET.FRC23 && sc < splat.length; sc++) {
					if (!minSphericalHarmonicsCoeff || splat[sc] < minSphericalHarmonicsCoeff) minSphericalHarmonicsCoeff = splat[sc];
					if (!maxSphericalHarmonicsCoeff || splat[sc] > maxSphericalHarmonicsCoeff) maxSphericalHarmonicsCoeff = splat[sc];
				}
			}
		}
		...
		for (let sa = 0; sa < splatArrays.length; sa++) {
			...
			for (let i = 0; i < splatArray.splatCount; i++) {
				...
				if ((targetSplat[UncompressedSplatArray.OFFSET.OPACITY] || 0) >= minimumAlpha) validSplats.addSplat(targetSplat);
			}
			...
			const bucketDataBytes = compressionLevel >= 1 ? buckets.length * SplatBuffer.BucketStorageSizeBytes + bucketMetaDataSizeBytes : 0;
			const sectionSizeBytes = sectionDataSizeBytes + bucketDataBytes;
			const sectionBuffer = new ArrayBuffer(sectionSizeBytes);
			const compressionScaleFactor = compressionScaleRange / (sectionBlockSize * .5);
			...
			for (let b = 0; b < buckets.length; b++) {
				...
				for (let i = 0; i < bucket.splats.length; i++) {
					...
					SplatBuffer.writeSplatDataToSectionBuffer(targetSplat, sectionBuffer, bufferOffset, compressionLevel, shDegree, bucketCenter, compressionScaleFactor, compressionScaleRange, minSphericalHarmonicsCoeff, maxSphericalHarmonicsCoeff);
					outSplatCount++;
				}
			}
			...
			if (compressionLevel >= 1) {
				...
				for (let pfb = 0; pfb < partiallyFullBucketLengths.length; pfb++) bucketMetaDataArray[pfb] = partiallyFullBucketLengths[pfb];
				const bucketArray = new Float32Array(sectionBuffer, bucketMetaDataSizeBytes, buckets.length * SplatBuffer.BucketStorageSizeFloats);
				for (let b = 0; b < buckets.length; b++) {
					const bucket = buckets[b];
					const base = b * 3;
		...
		return new SplatBuffer(unifiedBuffer);
	}
	static computeBucketsForUncompressedSplatArray(splatArray, blockSize, bucketSize) {
		...
		for (let i = 0; i < splatCount; i++) {
			...
			if (!bucket) partiallyFullBuckets[bucketId] = bucket = {
				"splats": [],
		...
		for (let bucketId in partiallyFullBuckets) if (partiallyFullBuckets.hasOwnProperty(bucketId)) {
			const bucket = partiallyFullBuckets[bucketId];
			if (bucket) partiallyFullBucketArray.push(bucket);
		}
		return {
...
var HeaderMagicBytes = new Uint8Array([
	112,
	108,
	121,
	10
]);
var HeaderEndTokenBytes = new Uint8Array([
	10,
	101,
	110,
	100,
...
var unpackRot = (result, value) => {
	const norm = 1 / (Math.sqrt(2) * .5);
	const a = (unpackUnorm(value >>> 20, 10) - .5) * norm;
	const b = (unpackUnorm(value >>> 10, 10) - .5) * norm;
	const c = (unpackUnorm(value, 10) - .5) * norm;
...
var PlayCanvasCompressedPlyParser = class PlayCanvasCompressedPlyParser {
	static decodeHeaderText(headerText) {
		...
		for (let i = 1; i < headerLines.length; ++i) {
			...
			switch (words[0]) {
				case "format":
					if (words[1] !== "binary_little_endian") throw new Error("Unsupported ply format");
					break;
				case "element":
					element = {
						name: words[1],
					...
					break;
				case "property": {
					if (!DataTypeMap.has(words[1])) throw new Error(`Unrecognized property data type '${words[1]}' in ply header`);
				...
				case HeaderEndToken:
					...
					break;
				default: throw new Error(`Unrecognized header value '${words[0]}' in ply header`);
			}
			if (done) break;
		}
		...
		if (shElement) {
			...
			if (shElement.properties.length >= 45) sphericalHarmonicsDegree = 3;
			else if (shElement.properties.length >= 24) sphericalHarmonicsDegree = 2;
			else if (shElement.properties.length >= 9) sphericalHarmonicsDegree = 1;
		}
		return {
			...
			"headerSizeBytes": headerText.indexOf(HeaderEndToken) + 10 + 1,
	...
	static decodeHeader(plyBuffer) {
		...
		*/
		const find = (buf, search) => {
			...
			for (i = 0; i <= endIndex; ++i) {
				for (j = 0; j < search.length; ++j) if (buf[i + j] !== search[j]) break;
		...
		*/
		...
		if (buf.length >= HeaderMagicBytes.length && !startsWith(buf, HeaderMagicBytes)) throw new Error("Invalid PLY header");
		endHeaderTokenOffset = find(buf, HeaderEndTokenBytes);
		if (endHeaderTokenOffset === -1) throw new Error("End of PLY header not found");
	...
	static readElementData(element, readBuffer, readOffset, fromIndex, toIndex, propertyFilter = null) {
		...
		for (let e = fromIndex; e <= toIndex; ++e) for (let j = 0; j < element.properties.length; ++j) {
			...
			if ((!property.storage || property.storage.byteLength < requiredStorageSizeBytes) && (!propertyFilter || propertyFilter(property.name))) property.storage = new StorageType(element.count);
			if (property.storage) switch (property.type) {
				case "char":
				...
				case "float":
					...
					break;
				case "double": property.storage[e] = dataView.getFloat64(readOffset, true);
			}
	...
	static readPly(plyBuffer, propertyFilter = null) {
		...
		let readIndex = PlayCanvasCompressedPlyParser.readElementData(header.chunkElement, plyBuffer, header.headerSizeBytes, null, null, propertyFilter);
	...
	static getElementStorageArrays(chunkElement, vertexElement, shElement) {
		...
		if (shElement) {
			...
			for (let i = 0; i < 45; i++) {
				...
				if (fRest) shStorageArrays[fRestKey] = fRest;
				else break;
			}
	...
	static decompressBaseSplat = function() {
		...
		return function(index, chunkSplatIndexOffset, positionArray, positionExtremes, scaleArray, scaleExtremes, rotationArray, colorExtremes, colorArray, outSplat) {
			...
			if (colorExtremes.minR && colorExtremes.maxR) outSplat[OFFSET.FDC0] = clamp(Math.round(lerp(colorExtremes.minR[chunkIndex], colorExtremes.maxR[chunkIndex], c.x) * 255), 0, 255);
			else outSplat[OFFSET.FDC0] = clamp(Math.floor(c.x * 255), 0, 255);
			if (colorExtremes.minG && colorExtremes.maxG) outSplat[OFFSET.FDC1] = clamp(Math.round(lerp(colorExtremes.minG[chunkIndex], colorExtremes.maxG[chunkIndex], c.y) * 255), 0, 255);
			else outSplat[OFFSET.FDC1] = clamp(Math.floor(c.y * 255), 0, 255);
			if (colorExtremes.minB && colorExtremes.maxB) outSplat[OFFSET.FDC2] = clamp(Math.round(lerp(colorExtremes.minB[chunkIndex], colorExtremes.maxB[chunkIndex], c.z) * 255), 0, 255);
			else outSplat[OFFSET.FDC2] = clamp(Math.floor(c.z * 255), 0, 255);
	...
	static decompressSphericalHarmonics = function() {
		...
		return function(index, shArray, outSphericalHarmonicsDegree, readSphericalHarmonicsDegree, outSplat) {
			...
			for (let j = 0; j < 3; ++j) for (let k = 0; k < 15; ++k) {
				const outIndex = shIndexMap[j * 15 + k];
				if (k < outSHCoeff && k < readSHCoeff) outSplat[UncompressedSplatArray.OFFSET.FRC0 + outIndex] = shArray[j * readSHCoeff + k][index] * (8 / 255) - 4;
			}
			return outSplat;
		};
	}();
	...
	static parseSphericalHarmonicsToUncompressedSplatArraySection(chunkElement, shElement, fromIndex, toIndex, vertexDataBuffer, vertexReadOffset, outSphericalHarmonicsDegree, readSphericalHarmonicsDegree, splatArray, propertyFilter = null) {
		...
		const { sh } = PlayCanvasCompressedPlyParser.getElementStorageArrays(chunkElement, void 0, shElement);
		const shArrays = Object.values(sh);
		for (let i = fromIndex; i <= toIndex; ++i) PlayCanvasCompressedPlyParser.decompressSphericalHarmonics(i, shArrays, outSphericalHarmonicsDegree, readSphericalHarmonicsDegree, splatArray.splats[i]);
	}
	static parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree) {
		...
		for (let i = 0; i < vertexElement.count; ++i) {
			...
			if (outSphericalHarmonicsDegree > 0) PlayCanvasCompressedPlyParser.decompressSphericalHarmonics(i, shArrays, outSphericalHarmonicsDegree, sphericalHarmonicsDegree, newSplat);
		}
		return splatArray;
	}
	static parseToUncompressedSplatBuffer(plyBuffer, outSphericalHarmonicsDegree) {
		const { chunkElement, vertexElement, shElement, sphericalHarmonicsDegree } = PlayCanvasCompressedPlyParser.readPly(plyBuffer);
		outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, sphericalHarmonicsDegree);
		const { splatBuffer, splatBufferDataOffsetBytes } = SplatBuffer.preallocateUncompressed(vertexElement.count, outSphericalHarmonicsDegree);
		const { positionExtremes, scaleExtremes, colorExtremes, position, rotation, scale, color } = PlayCanvasCompressedPlyParser.getElementStorageArrays(chunkElement, vertexElement);
		let shArrays;
		if (outSphericalHarmonicsDegree > 0) {
			const { sh } = PlayCanvasCompressedPlyParser.getElementStorageArrays(chunkElement, void 0, shElement);
...
var [FieldSizeIdDouble, FieldSizeIdInt, FieldSizeIdUInt, FieldSizeIdFloat, FieldSizeIdShort, FieldSizeIdUShort, FieldSizeIdUChar] = [
	0,
	1,
	2,
	3,
...
var PlyParserUtils = class PlyParserUtils {
	...
	static decodeSectionHeader(headerLines, fieldNameIdMap, headerStartLine = 0) {
				...
				if (processingSection) {
					...
					for (let lineComponent of lineComponents) {
						...
						if (trimmedComponent.length > 0) {
							validComponents++;
							if (validComponents === 2) sectionName = trimmedComponent;
							else if (validComponents === 3) vertexCount = parseInt(trimmedComponent);
						}
					}
				}
		...
		for (let fieldName of allFieldNames) {
			...
			if (fieldTypesByName.hasOwnProperty(fieldName)) {
				const fieldId = fieldNameIdMap[fieldName];
				if (fieldId !== void 0) fieldOffsets[fieldId] = bytesPerVertex;
			}
	...
	static decodeSphericalHarmonicsFromSectionHeader(fieldNames, fieldNameIdMap) {
		...
		for (let fieldName of fieldNames) if (fieldName.startsWith("f_rest")) sphericalHarmonicsFieldCount++;
		...
		for (let rgb = 0; rgb < 3; rgb++) {
			if (degree >= 1) for (let i = 0; i < 3; i++) degree1Fields.push(fieldNameIdMap["f_rest_" + (i + coefficientsPerChannel * rgb)]);
			if (degree >= 2) for (let i = 0; i < 5; i++) degree2Fields.push(fieldNameIdMap["f_rest_" + (i + coefficientsPerChannel * rgb + 3)]);
		}
		return {
			"degree": degree,
	...
	static getHeaderSectionNames(headerLines) {
		const sectionNames = [];
		for (let headerLine of headerLines) if (headerLine.startsWith("element")) {
			...
			for (let lineComponent of lineComponents) {
				...
				if (trimmedComponent.length > 0) {
					validComponents++;
					if (validComponents === 2) sectionNames.push(trimmedComponent);
				}
			}
		}
		return sectionNames;
	...
	static checkTextForEndHeader(endHeaderTestText) {
		if (endHeaderTestText.includes(PlyParserUtils.HeaderEndToken)) return true;
		return false;
	}
	...
	static extractHeaderFromBufferToText(plyBuffer) {
		...
		while (true) {
			if (headerOffset + readChunkSize >= plyBuffer.byteLength) throw new Error("End of file reached while searching for end of header");
			const headerChunk = new Uint8Array(plyBuffer, headerOffset, readChunkSize);
			headerText += decoder.decode(headerChunk);
			headerOffset += readChunkSize;
			if (PlyParserUtils.checkBufferForEndHeader(plyBuffer, headerOffset, 200, decoder)) break;
		}
		return headerText;
	}
	...
	static convertHeaderTextToLines(headerText) {
		...
		for (let i = 0; i < headerLines.length; i++) {
			...
			if (line === PlyParserUtils.HeaderEndToken) break;
		}
		return prunedLines;
	}
	static determineHeaderFormatFromHeaderText(headertText) {
		...
		for (let i = 0; i < headerLines.length; i++) {
			const line = headerLines[i].trim();
			if (line.startsWith("element chunk") || line.match(/[A-Za-z]*packed_[A-Za-z]*/)) format = PlyFormat.PlayCanvasCompressed;
			else if (line.startsWith("element codebook_centers")) format = PlyFormat.INRIAV2;
			else if (line === PlyParserUtils.HeaderEndToken) break;
		}
		return format;
	}
	...
	static readVertex(vertexData, header, row, dataOffset, fieldsToRead, rawVertex, normalize = true) {
		...
		for (let fieldId of fieldsToRead) {
			...
			if (fieldType === FieldSizeIdFloat) rawVertex[fieldId] = vertexData.getFloat32(offset + fieldOffsets[fieldId], true);
			else if (fieldType === FieldSizeIdShort) rawVertex[fieldId] = vertexData.getInt16(offset + fieldOffsets[fieldId], true);
			else if (fieldType === FieldSizeIdUShort) rawVertex[fieldId] = vertexData.getUint16(offset + fieldOffsets[fieldId], true);
			else if (fieldType === FieldSizeIdInt) rawVertex[fieldId] = vertexData.getInt32(offset + fieldOffsets[fieldId], true);
			else if (fieldType === FieldSizeIdUInt) rawVertex[fieldId] = vertexData.getUint32(offset + fieldOffsets[fieldId], true);
			else if (fieldType === FieldSizeIdUChar) {
				if (normalize) rawVertex[fieldId] = vertexData.getUint8(offset + fieldOffsets[fieldId]) / 255;
				else rawVertex[fieldId] = vertexData.getUint8(offset + fieldOffsets[fieldId]);
			}
		}
	}
};
...
var [SCALE_0, SCALE_1, SCALE_2, ROT_0, ROT_1, ROT_2, ROT_3, X, Y, Z, F_DC_0, F_DC_1, F_DC_2, OPACITY, RED$1, GREEN$1, BLUE$1, F_REST_0] = BaseFieldNamesToRead.map((e, i) => i);
var INRIAV1PlyParser = class INRIAV1PlyParser {
	static decodeHeaderLines(headerLines) {
		...
		if (shLineCount >= 45) shFieldsToReadCount = 45;
		else if (shLineCount >= 24) shFieldsToReadCount = 24;
		else if (shLineCount >= 9) shFieldsToReadCount = 9;
		let shRemainingFieldNamesToRead = Array.from(Array(Math.max(shFieldsToReadCount - 1, 0))).map((element, index) => `f_rest_${index + 1}`);
	...
	static parseToUncompressedSplatBufferSection(header, fromSplat, toSplat, splatData, splatDataOffset, toBuffer, toOffset, outSphericalHarmonicsDegree = 0) {
		outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
		const outBytesPerSplat = SplatBuffer.CompressionLevels[0].SphericalHarmonicsDegrees[outSphericalHarmonicsDegree].BytesPerSplat;
		for (let i = fromSplat; i <= toSplat; i++) {
			const parsedSplat = INRIAV1PlyParser.parseToUncompressedSplat(splatData, i, header, splatDataOffset, outSphericalHarmonicsDegree);
	...
	static parseToUncompressedSplatArraySection(header, fromSplat, toSplat, splatData, splatDataOffset, splatArray, outSphericalHarmonicsDegree = 0) {
		outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
		for (let i = fromSplat; i <= toSplat; i++) {
			const parsedSplat = INRIAV1PlyParser.parseToUncompressedSplat(splatData, i, header, splatDataOffset, outSphericalHarmonicsDegree);
	...
	static decodeSectionSplatData(sectionSplatData, splatCount, sectionHeader, outSphericalHarmonicsDegree, toSplatArray = true) {
		...
		if (toSplatArray) {
			...
			for (let row = 0; row < splatCount; row++) {
				const newSplat = INRIAV1PlyParser.parseToUncompressedSplat(sectionSplatData, row, sectionHeader, 0, outSphericalHarmonicsDegree);
				splatArray.addSplat(newSplat);
			}
			return splatArray;
		} else {
			const { splatBuffer, splatBufferDataOffsetBytes } = SplatBuffer.preallocateUncompressed(splatCount, outSphericalHarmonicsDegree);
			INRIAV1PlyParser.parseToUncompressedSplatBufferSection(sectionHeader, 0, splatCount - 1, sectionSplatData, 0, splatBuffer.bufferData, splatBufferDataOffsetBytes, outSphericalHarmonicsDegree);
			return splatBuffer;
		}
	}
	static parseToUncompressedSplat = function() {
		...
		const OFFSET_FRC = [];
		for (let i = 0; i < 45; i++) OFFSET_FRC[i] = UncompressedSplatArray.OFFSET.FRC0 + i;
		return function(splatData, row, header, splatDataOffset = 0, outSphericalHarmonicsDegree = 0) {
			...
			if (rawSplat[SCALE_0] !== void 0) {
				...
				newSplat[OFFSET_SCALE0] = .01;
				newSplat[OFFSET_SCALE1] = .01;
				newSplat[OFFSET_SCALE2] = .01;
			}
			if (rawSplat[F_DC_0] !== void 0) {
				const SH_C0 = .28209479177387814;
				newSplat[OFFSET_FDC0] = (.5 + SH_C0 * rawSplat[F_DC_0]) * 255;
				newSplat[OFFSET_FDC1] = (.5 + SH_C0 * rawSplat[F_DC_1]) * 255;
				newSplat[OFFSET_FDC2] = (.5 + SH_C0 * rawSplat[F_DC_2]) * 255;
			} else if (rawSplat[RED$1] !== void 0) {
			...
			if (rawSplat[OPACITY] !== void 0) newSplat[OFFSET_OPACITY] = 1 / (1 + Math.exp(-rawSplat[OPACITY])) * 255;
			...
			if (outSphericalHarmonicsDegree >= 1) {
				if (rawSplat[F_REST_0] !== void 0) {
					for (let i = 0; i < 9; i++) newSplat[OFFSET_FRC[i]] = rawSplat[header.sphericalHarmonicsDegree1Fields[i]];
					if (outSphericalHarmonicsDegree >= 2) for (let i = 0; i < 15; i++) newSplat[OFFSET_FRC[9 + i]] = rawSplat[header.sphericalHarmonicsDegree2Fields[i]];
				}
			}
...
function separatePlyHeaderAndData(plyBuffer) {
	...
	return {
		...
		splatData: INRIAV1PlyParser.findSplatData(plyBuffer, header)
	};
}
...
var [CB_FEATURES_DC, CB_FEATURES_REST_0, CB_FEATURES_REST_3, CB_OPACITY, CB_SCALING, CB_ROTATION_RE, CB_ROTATION_IM] = [
	0,
	1,
	4,
	16,
...
var [PLY_SCALE_0, PLY_SCALE_1, PLY_SCALE_2, PLY_ROT_0, PLY_ROT_1, PLY_ROT_2, PLY_ROT_3, PLY_X, PLY_Y, PLY_Z, PLY_F_DC_0, PLY_F_DC_1, PLY_F_DC_2, PLY_OPACITY] = FieldsToReadIndexes;
...
var INRIAV2PlyParser = class INRIAV2PlyParser {
	static decodeSectionHeadersFromHeaderLines(headerLines) {
		...
		for (let s = 0; s < sectionNames.length; s++) if (sectionNames[s] === "codebook_centers") codeBookSectionIndex = s;
		...
		while (!lastSectionFound) {
			let sectionHeader;
			if (sectionIndex === codeBookSectionIndex) sectionHeader = PlyParserUtils.decodeSectionHeader(headerLines, codeBookEntriesToReadIdMap, currentStartLine);
			else sectionHeader = PlyParserUtils.decodeSectionHeader(headerLines, fieldNameIdMap, currentStartLine);
	...
	static getSplatCountFromSectionHeaders(sectionHeaders) {
		let splatCount = 0;
		for (let sectionHeader of sectionHeaders) if (sectionHeader.sectionName !== "codebook_centers") splatCount += sectionHeader.vertexCount;
		return splatCount;
	}
	static decodeHeaderFromHeaderText(headerText) {
		...
		return {
			...
			"splatCount": INRIAV2PlyParser.getSplatCountFromSectionHeaders(sectionHeaders)
		};
	}
	static decodeHeaderFromBuffer(plyBuffer) {
		const headerText = PlyParserUtils.readHeaderFromBuffer(plyBuffer);
	...
	static decodeCodeBook(codeBookData, sectionHeader) {
		...
		for (let row = 0; row < sectionHeader.vertexCount; row++) {
			...
			for (let index of CodeBookEntriesToReadIndexes) {
				...
				if (!codeBookPage) codeBook[codeBookElementOffset] = codeBookPage = [];
		...
		for (let page = 0; page < codeBook.length; page++) {
			...
			for (let i = 0; i < codeBookPage.length; i++) {
				...
				if (page === CB_OPACITY) codeBookPage[i] = Math.round(1 / (1 + Math.exp(-baseValue)) * 255);
				else if (page === CB_FEATURES_DC) codeBookPage[i] = Math.round((.5 + SH_C0 * baseValue) * 255);
				else if (page === CB_SCALING) codeBookPage[i] = Math.exp(baseValue);
				else codeBookPage[i] = baseValue;
			}
		}
		return codeBook;
	}
	static decodeSectionSplatData(sectionSplatData, splatCount, sectionHeader, codeBook, outSphericalHarmonicsDegree) {
		...
		const splatArray = new UncompressedSplatArray(outSphericalHarmonicsDegree);
		for (let row = 0; row < splatCount; row++) {
			const newSplat = INRIAV2PlyParser.parseToUncompressedSplat(sectionSplatData, row, sectionHeader, codeBook, 0, outSphericalHarmonicsDegree);
			splatArray.addSplat(newSplat);
		}
		return splatArray;
	}
	static parseToUncompressedSplat = function() {
		...
		return function(splatData, row, header, codeBook, splatDataOffset = 0, outSphericalHarmonicsDegree = 0) {
			...
			if (rawSplat[PLY_OPACITY] !== void 0) newSplat[OFFSET_OPACITY] = codeBook[CB_OPACITY][rawSplat[PLY_OPACITY]];
			newSplat[OFFSET_FDC0] = clamp(Math.floor(newSplat[OFFSET_FDC0]), 0, 255);
			newSplat[OFFSET_FDC1] = clamp(Math.floor(newSplat[OFFSET_FDC1]), 0, 255);
			newSplat[OFFSET_FDC2] = clamp(Math.floor(newSplat[OFFSET_FDC2]), 0, 255);
			newSplat[OFFSET_OPACITY] = clamp(Math.floor(newSplat[OFFSET_OPACITY]), 0, 255);
			if (outSphericalHarmonicsDegree >= 1 && header.sphericalHarmonicsDegree >= 1) {
				...
				if (outSphericalHarmonicsDegree >= 2 && header.sphericalHarmonicsDegree >= 2) for (let i = 0; i < 15; i++) {
	...
	static parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree = 0) {
		...
		for (let s = 0; s < header.sectionHeaders.length; s++) {
			...
			if (sectionHeader.sectionName !== "codebook_centers") {
				...
				const splatArray = INRIAV2PlyParser.decodeSectionSplatData(vertexData, splatCount, sectionHeader, codeBook, outSphericalHarmonicsDegree);
				splatArrays.push(splatArray);
			}
		}
		const unified = new UncompressedSplatArray(outSphericalHarmonicsDegree);
		for (let splatArray of splatArrays) for (let splat of splatArray.splats) unified.addSplat(splat);
		return unified;
	}
};
var PlyParser = class {
	static parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree = 0) {
		...
		if (plyFormat === PlyFormat.PlayCanvasCompressed) return PlayCanvasCompressedPlyParser.parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree);
		else if (plyFormat === PlyFormat.INRIAV1) return INRIAV1PlyParser.parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree);
		else if (plyFormat === PlyFormat.INRIAV2) return INRIAV2PlyParser.parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree);
	}
	static parseToUncompressedSplatBuffer(plyBuffer, outSphericalHarmonicsDegree = 0) {
		const plyFormat = PlyParserUtils.determineHeaderFormatFromPlyBuffer(plyBuffer);
		if (plyFormat === PlyFormat.PlayCanvasCompressed) return PlayCanvasCompressedPlyParser.parseToUncompressedSplatBuffer(plyBuffer, outSphericalHarmonicsDegree);
		else if (plyFormat === PlyFormat.INRIAV1) return INRIAV1PlyParser.parseToUncompressedSplatBuffer(plyBuffer, outSphericalHarmonicsDegree);
		else if (plyFormat === PlyFormat.INRIAV2) throw new Error("parseToUncompressedSplatBuffer() is not implemented for INRIA V2 PLY files");
	}
};
var SplatPartitioner = class SplatPartitioner {
	...
	partitionUncompressedSplatArray(splatArray) {
		...
		for (let s = 0; s < sectionCount; s++) {
			...
			for (let i = 0; i < splatArray.splatCount; i++) if (sectionFilter(i)) sectionSplats.addSplat(splatArray.splats[i]);
	...
	static getStandardPartitioner(partitionSize = 0, sceneCenter = new Vector3(), blockSize = SplatBuffer.BucketBlockSize, bucketSize = SplatBuffer.BucketSize) {
		const partitionGenerator = (splatArray) => {
			...
			splatArray.splats.sort((a, b) => {
				if (a.centerDist > b.centerDist) return 1;
				else return -1;
			});
		...
		return new SplatPartitioner(void 0, void 0, void 0, partitionGenerator);
	}
};
var SplatBufferGenerator = class SplatBufferGenerator {
	...
	generateFromUncompressedSplatArray(splatArray) {
		...
		return SplatBuffer.generateFromUncompressedSplatArrays(partitionResults.splatArrays, this.alphaRemovalThreshold, this.compressionLevel, this.sceneCenter, this.blockSize, this.bucketSize, partitionResults.parameters);
	}
...
var DirectLoadError = class extends Error {
	constructor(msg) {
		super(msg);
	}
};
var InternalLoadType = {
...
function storeChunksInBuffer(chunks, buffer) {
	...
	if (!buffer || buffer.byteLength < inBytes) buffer = new ArrayBuffer(inBytes);
	let offset = 0;
...
function finalize$1(splatData, optimizeSplatData, minimumAlpha, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize) {
	if (optimizeSplatData) return SplatBufferGenerator.getStandardGenerator(minimumAlpha, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize).generateFromUncompressedSplatArray(splatData);
	else return SplatBuffer.generateFromUncompressedSplatArrays([splatData], minimumAlpha, 0, new Vector3());
}
var PlyLoader = class PlyLoader {
	static loadFromURL(fileName, onProgress, progressiveLoadToSplatBuffer, onProgressiveLoadSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, outSphericalHarmonicsDegree = 0, headers, sectionSize, sceneCenter, blockSize, bucketSize) {
		...
		if (!progressiveLoadToSplatBuffer && !optimizeSplatData) internalLoadType = InternalLoadType.DownloadBeforeProcessing;
		else if (optimizeSplatData) internalLoadType = InternalLoadType.ProgressiveToSplatArray;
		...
		const localOnProgress = (percent, percentLabel, chunkData) => {
			...
			if (internalLoadType === InternalLoadType.DownloadBeforeProcessing) {
				if (loadComplete) loadPromise.resolve(chunks);
			} else {
				if (!headerLoaded) {
					...
					if (PlyParserUtils.checkTextForEndHeader(headerText)) {
						...
						if (plyFormat === PlyFormat.INRIAV1) {
							...
							outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
							if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer && outSphericalHarmonicsDegree > 0) throw new DirectLoadError("PlyLoader.loadFromURL() -> Selected PLY format has spherical harmonics data that cannot be progressively loaded.");
							maxSplatCount = header.vertexElement.count;
							endOfBaseSplatDataBytes = header.headerSizeBytes + header.bytesPerSplat * maxSplatCount + header.chunkElement.storageSizeBytes;
						} else if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) throw new DirectLoadError("PlyLoader.loadFromURL() -> Selected PLY format cannot be progressively loaded.");
						else {
						...
						} else standardLoadUncompressedSplatArray = new UncompressedSplatArray(outSphericalHarmonicsDegree);
					...
					if (compressedPlyHeaderChunksBuffer.byteLength >= sizeRequiredForHeaderAndChunks) {
						PlayCanvasCompressedPlyParser.readElementData(header.chunkElement, compressedPlyHeaderChunksBuffer, header.headerSizeBytes);
				...
				if (headerLoaded && readyToLoadSplatData && chunks.length > 0) {
					...
					if (numBytesDownloaded - numBytesStreamed > directLoadSectionSizeBytes || numBytesDownloaded >= endOfBaseSplatDataBytes && !baseSplatDataLoaded || loadComplete) {
						const bytesPerSplat = baseSplatDataLoaded ? header.sphericalHarmonicsPerSplat : header.bytesPerSplat;
						const numBytesToProcess = (baseSplatDataLoaded ? numBytesDownloaded : Math.min(endOfBaseSplatDataBytes, numBytesDownloaded)) - numBytesParsed;
						...
						if (!baseSplatDataLoaded) {
							if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) {
								...
								if (plyFormat === PlyFormat.PlayCanvasCompressed) PlayCanvasCompressedPlyParser.parseToUncompressedSplatBufferSection(header.chunkElement, header.vertexElement, 0, addedSplatCount - 1, processedBaseSplatCount, dataToParse, directLoadBufferOut, outOffset);
								else INRIAV1PlyParser.parseToUncompressedSplatBufferSection(header, 0, addedSplatCount - 1, dataToParse, 0, directLoadBufferOut, outOffset, outSphericalHarmonicsDegree);
							} else if (plyFormat === PlyFormat.PlayCanvasCompressed) PlayCanvasCompressedPlyParser.parseToUncompressedSplatArraySection(header.chunkElement, header.vertexElement, 0, addedSplatCount - 1, processedBaseSplatCount, dataToParse, standardLoadUncompressedSplatArray);
							else INRIAV1PlyParser.parseToUncompressedSplatArraySection(header, 0, addedSplatCount - 1, dataToParse, 0, standardLoadUncompressedSplatArray, outSphericalHarmonicsDegree);
				...
				if (onProgressiveLoadSectionProgress && directLoadSplatBuffer) onProgressiveLoadSectionProgress(directLoadSplatBuffer, loadComplete);
				if (loadComplete) {
					if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) loadPromise.resolve(directLoadSplatBuffer);
					else loadPromise.resolve(standardLoadUncompressedSplatArray);
				}
			}
		...
		return fetchWithProgress(fileName, localOnProgress, false, headers).then(() => {
			...
			return loadPromise.promise.then((splatData) => {
				...
				} else if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) return splatData;
				else return delayedExecute(() => {
					return finalize$1(splatData, optimizeSplatData, minimumAlpha, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize);
				});
			});
		});
	}
	static loadFromFileData(plyFileData, minimumAlpha, compressionLevel, optimizeSplatData, outSphericalHarmonicsDegree = 0, sectionSize, sceneCenter, blockSize, bucketSize) {
		if (optimizeSplatData) return delayedExecute(() => {
			return PlyParser.parseToUncompressedSplatArray(plyFileData, outSphericalHarmonicsDegree);
		}).then((splatArray) => {
			return finalize$1(splatArray, optimizeSplatData, minimumAlpha, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize);
		});
		else return delayedExecute(() => {
...
function halfToFloat(h) {
	...
	if (exponent === 0) return signMul * Math.pow(2, -14) * mantissa / 1024;
	if (exponent === 31) return mantissa !== 0 ? NaN : signMul * Infinity;
...
var unpackedSplatToUncompressedSplat = function() {
	let rawSplat = [];
	const tempRotation = new Quaternion();
	const OFFSET_X = UncompressedSplatArray.OFFSET.X;
	const OFFSET_Y = UncompressedSplatArray.OFFSET.Y;
	...
	return function(unpackedSplat, unpackedSphericalHarmonicsDegree, outSphericalHarmonicsDegree) {
		...
		const newSplat = UncompressedSplatArray.createSplat(outSphericalHarmonicsDegree);
		if (unpackedSplat.scale[0] !== void 0) {
		...
		if (unpackedSplat.color[0] !== void 0) {
		...
		if (unpackedSplat.alpha !== void 0) newSplat[OFFSET_OPACITY] = unpackedSplat.alpha;
		newSplat[OFFSET_FDC0] = clamp(Math.floor(newSplat[OFFSET_FDC0]), 0, 255);
		newSplat[OFFSET_FDC1] = clamp(Math.floor(newSplat[OFFSET_FDC1]), 0, 255);
		newSplat[OFFSET_FDC2] = clamp(Math.floor(newSplat[OFFSET_FDC2]), 0, 255);
		newSplat[OFFSET_OPACITY] = clamp(Math.floor(newSplat[OFFSET_OPACITY]), 0, 255);
		...
		for (let j = 0; j < 3; ++j) for (let k = 0; k < 15; ++k) {
			const outIndex = shIndexMap[j * 15 + k];
			if (k < outSHCoeff && k < readSHCoeff) newSplat[UncompressedSplatArray.OFFSET.FRC0 + outIndex] = unpackedSplat.sh[j * readSHCoeff + k];
		}
...
function unpackGaussians(packed, outSphericalHarmonicsDegree, directToSplatBuffer, outTarget, outTargetOffset) {
	...
	if (!checkSizes2(packed, numPoints, shDim, usesFloat16)) return null;
	const splat = {
		position: [],
		scale: [],
		rotation: [],
	...
	let halfData;
	if (usesFloat16) halfData = new Uint16Array(packed.positions.buffer, packed.positions.byteOffset, numPoints * 3);
	...
	for (let i = 0; i < numPoints; i++) {
		if (usesFloat16) for (let j = 0; j < 3; j++) splat.position[j] = halfToFloat(halfData[i * 3 + j]);
			...
			fixed32 |= fixed32 & 8388608 ? 4278190080 : 0;
			splat.position[j] = fixed32 * fullPrecisionPositionScale;
		}
		for (let j = 0; j < 3; j++) splat.scale[j] = Math.exp(packed.scales[i * 3 + j] / 16 - 10);
		...
		splat.rotation[3] = Math.sqrt(Math.max(0, 1 - squaredNorm));
		splat.alpha = Math.floor(packed.alphas[i]);
		for (let j = 0; j < 3; j++) splat.color[j] = Math.floor(((packed.colors[i * 3 + j] / 255 - .5) / COLOR_SCALE * SH_C0 + .5) * 255);
		for (let j = 0; j < 3; j++) for (let k = 0; k < shCoeffPerChannelPerSplat; k++) splat.sh[j * shCoeffPerChannelPerSplat + k] = unquantizeSH(packed.sh[shCoeffPerChannelPerSplat * 3 * i + k * 3 + j]);
		...
		} else outTarget.addSplat(uncompressedSplat);
	}
}
var HEADER_SIZE = 16;
var MAX_POINTS_TO_READ = 1e7;
...
async function loadSpzPacked(compressedData) {
	try {
		return deserializePackedGaussians((await decompressGzipped(compressedData)).buffer);
	} catch (error) {
		console.error("[SPZ ERROR] loadSpzPacked: decompression error", error);
		return null;
	}
}
var SpzLoader = class SpzLoader {
	static loadFromURL(fileName, onProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, outSphericalHarmonicsDegree = 0, headers, sectionSize, sceneCenter, blockSize, bucketSize) {
		if (onProgress) onProgress(0, "0%", LoaderStatus.Downloading);
		return fetchWithProgress(fileName, onProgress, true, headers).then((fileData) => {
			if (onProgress) onProgress(0, "0%", LoaderStatus.Processing);
			return SpzLoader.loadFromFileData(fileData, minimumAlpha, compressionLevel, optimizeSplatData, outSphericalHarmonicsDegree, sectionSize, sceneCenter, blockSize, bucketSize);
		});
	}
	static async loadFromFileData(spzFileData, minimumAlpha, compressionLevel, optimizeSplatData, outSphericalHarmonicsDegree = 0, sectionSize, sceneCenter, blockSize, bucketSize) {
		...
		if (optimizeSplatData) {
			...
			return SplatBufferGenerator.getStandardGenerator(minimumAlpha, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize).generateFromUncompressedSplatArray(splatArray);
		} else {
			const { splatBuffer, splatBufferDataOffsetBytes } = SplatBuffer.preallocateUncompressed(packed.numPoints, outSphericalHarmonicsDegree);
			unpackGaussians(packed, outSphericalHarmonicsDegree, true, splatBuffer.bufferData, splatBufferDataOffsetBytes);
			return splatBuffer;
		}
	}
};
var SplatParser = class SplatParser {
	...
	static parseToUncompressedSplatBufferSection(fromSplat, toSplat, fromBuffer, fromOffset, toBuffer, toOffset) {
		...
		for (let i = fromSplat; i <= toSplat; i++) {
			...
			const inRotation = new Uint8Array(fromBuffer, inBase + SplatParser.CenterSizeBytes + SplatParser.ScaleSizeBytes + SplatParser.RotationSizeBytes, 4);
			const quat = new Quaternion((inRotation[1] - 128) / 128, (inRotation[2] - 128) / 128, (inRotation[3] - 128) / 128, (inRotation[0] - 128) / 128);
			quat.normalize();
			const outBase = i * outBytesPerSplat + toOffset;
	...
	static parseToUncompressedSplatArraySection(fromSplat, toSplat, fromBuffer, fromOffset, splatArray) {
		for (let i = fromSplat; i <= toSplat; i++) {
			...
			const quat = new Quaternion((inRotation[1] - 128) / 128, (inRotation[2] - 128) / 128, (inRotation[3] - 128) / 128, (inRotation[0] - 128) / 128);
			quat.normalize();
			splatArray.addSplatFromComonents(inCenter[0], inCenter[1], inCenter[2], inScale[0], inScale[1], inScale[2], quat.w, quat.x, quat.y, quat.z, inColor[0], inColor[1], inColor[2], inColor[3]);
		}
	}
	static parseStandardSplatToUncompressedSplatArray(inBuffer) {
		...
		for (let i = 0; i < splatCount; i++) {
			...
			const inRotation = new Uint8Array(inBuffer, inBase + SplatParser.CenterSizeBytes + SplatParser.ScaleSizeBytes + SplatParser.ColorSizeBytes, 4);
			const quat = new Quaternion((inRotation[1] - 128) / 128, (inRotation[2] - 128) / 128, (inRotation[3] - 128) / 128, (inRotation[0] - 128) / 128);
			quat.normalize();
			splatArray.addSplatFromComonents(inCenter[0], inCenter[1], inCenter[2], inScale[0], inScale[1], inScale[2], quat.w, quat.x, quat.y, quat.z, inColor[0], inColor[1], inColor[2], inColor[3]);
		}
...
var SplatLoader = class SplatLoader {
	static loadFromURL(fileName, onProgress, progressiveLoadToSplatBuffer, onProgressiveLoadSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, headers, sectionSize, sceneCenter, blockSize, bucketSize) {
		let internalLoadType = progressiveLoadToSplatBuffer ? InternalLoadType.ProgressiveToSplatBuffer : InternalLoadType.ProgressiveToSplatArray;
		...
		const localOnProgress = (percent, percentStr, chunk, fileSize) => {
			...
			if (!fileSize) {
				if (progressiveLoadToSplatBuffer) throw new DirectLoadError("Cannon directly load .splat because no file size info is available.");
				else {
					internalLoadType = InternalLoadType.DownloadBeforeProcessing;
					return;
				}
			...
			if (!directLoadBufferIn) {
				...
				} else standardLoadUncompressedSplatArray = new UncompressedSplatArray(0);
			}
			if (chunk) {
				...
				if (bytesLoadedSinceLastSection > directLoadSectionSizeBytes || loadComplete) {
					const addedSplatCount = (loadComplete ? bytesLoadedSinceLastSection : directLoadSectionSizeBytes) / SplatParser.RowSizeBytes;
					const newSplatCount = splatCount + addedSplatCount;
					if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) SplatParser.parseToUncompressedSplatBufferSection(splatCount, newSplatCount - 1, directLoadBufferIn, 0, directLoadBufferOut, splatDataOffsetBytes);
					else SplatParser.parseToUncompressedSplatArraySection(splatCount, newSplatCount - 1, directLoadBufferIn, 0, standardLoadUncompressedSplatArray);
					...
					if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) {
						...
						if (onProgressiveLoadSectionProgress) onProgressiveLoadSectionProgress(directLoadSplatBuffer, loadComplete);
					}
					numBytesStreamed += directLoadSectionSizeBytes;
				}
			}
		...
		return fetchWithProgress(fileName, localOnProgress, false, headers).then(() => {
			...
			return loadPromise.promise.then((splatData) => {
				if (onProgress) onProgress(100, "100%", LoaderStatus.Done);
				if (internalLoadType === InternalLoadType.DownloadBeforeProcessing) return new Blob(chunks).arrayBuffer().then((splatData) => {
					return SplatLoader.loadFromFileData(splatData, minimumAlpha, compressionLevel, optimizeSplatData, sectionSize, sceneCenter, blockSize, bucketSize);
				});
				else if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) return splatData;
				else return delayedExecute(() => {
	...
	static loadFromFileData(splatFileData, minimumAlpha, compressionLevel, optimizeSplatData, sectionSize, sceneCenter, blockSize, bucketSize) {
		return delayedExecute(() => {
			return finalize(SplatParser.parseStandardSplatToUncompressedSplatArray(splatFileData), optimizeSplatData, minimumAlpha, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize);
		});
	}
};
var KSplatLoader = class KSplatLoader {
	static checkVersion(buffer) {
		...
		if (header.versionMajor === minVersionMajor && header.versionMinor >= minVersionMinor || header.versionMajor > minVersionMajor) return true;
		else throw new Error(`KSplat version not supported: v${header.versionMajor}.${header.versionMinor}. Minimum required: v${minVersionMajor}.${minVersionMinor}`);
	}
	static loadFromURL(fileName, externalOnProgress, progressiveLoadToSplatBuffer, onSectionBuilt, headers) {
		...
		const checkAndLoadHeader = () => {
			if (!headerLoaded && !headerLoading && numBytesLoaded >= SplatBuffer.HeaderSizeBytes) {
				headerLoading = true;
				new Blob(chunks).arrayBuffer().then((bufferData) => {
		...
		const checkAndLoadSectionHeaders = () => {
			const performLoad = () => {
				...
				new Blob(chunks).arrayBuffer().then((bufferData) => {
					...
					new Uint8Array(sectionHeadersBuffer).set(new Uint8Array(bufferData, SplatBuffer.HeaderSizeBytes, header.maxSectionCount * SplatBuffer.SectionHeaderSizeBytes));
					sectionHeaders = SplatBuffer.parseSectionHeaders(header, sectionHeadersBuffer, 0, false);
					let totalSectionStorageStorageByes = 0;
					for (let i = 0; i < header.maxSectionCount; i++) totalSectionStorageStorageByes += sectionHeaders[i].storageSizeBytes;
					...
					for (let i = 0; i <= sectionHeaders.length && i < header.maxSectionCount; i++) totalBytesToDownload += sectionHeaders[i].storageSizeBytes;
					queueCheckAndLoadSections();
				});
			};
			if (!sectionHeadersLoading && !sectionHeadersLoaded && headerLoaded && numBytesLoaded >= SplatBuffer.HeaderSizeBytes + SplatBuffer.SectionHeaderSizeBytes * header.maxSectionCount) performLoad();
		};
		const checkAndLoadSections = () => {
			...
			const checkAndLoadFunc = () => {
				...
				if (sectionHeadersLoaded) {
					...
					if (numBytesLoaded - numBytesProgressivelyLoaded > Constants.ProgressiveLoadSectionSize || downloadComplete) {
						...
						for (let i = 0; i < header.maxSectionCount; i++) {
							...
							const bytesRequiredToReachSectionSplatData = baseDataOffset + (sectionBase + sectionHeader.partiallyFilledBucketCount * 4 + sectionHeader.bucketStorageSizeBytes * sectionHeader.bucketCount);
							if (numBytesProgressivelyLoaded >= bytesRequiredToReachSectionSplatData) {
								...
								const bytesPerSplat = SplatBuffer.CompressionLevels[header.compressionLevel].SphericalHarmonicsDegrees[sectionHeader.sphericalHarmonicsDegree].BytesPerSplat;
						...
						if (loadComplete) directLoadPromise.resolve(directLoadSplatBuffer);
						else checkAndLoadSections();
					}
				}
			};
		...
		const localOnProgress = (percent, percentStr, chunk) => {
			if (chunk) {
				chunks.push(chunk);
				if (directLoadBuffer) new Uint8Array(directLoadBuffer, numBytesLoaded, chunk.byteLength).set(new Uint8Array(chunk));
				numBytesLoaded += chunk.byteLength;
			}
			if (progressiveLoadToSplatBuffer) {
				...
				checkAndLoadSectionHeaders();
				checkAndLoadSections();
			} else if (externalOnProgress) externalOnProgress(percent, percentStr, LoaderStatus.Downloading);
		};
		return fetchWithProgress(fileName, localOnProgress, !progressiveLoadToSplatBuffer, headers).then((fullBuffer) => {
			...
			return (progressiveLoadToSplatBuffer ? directLoadPromise.promise : KSplatLoader.loadFromFileData(fullBuffer)).then((splatBuffer) => {
	...
	static downloadFile = function() {
		...
		return function(splatBuffer, fileName) {
			const blob = new Blob([splatBuffer.bufferData], { type: "application/octet-stream" });
			if (!downLoadLink) {
		...
		this.update = function() {
			...
			return function update() {
				...
				if (scope.zoomToCursor && performCursorZoom) {
					...
					if (newRadius !== null) {
						if (this.screenSpacePanning) scope.target.set(0, 0, -1).transformDirection(scope.object.matrix).multiplyScalar(newRadius).add(scope.object.position);
						else {
...
var fadeElement = (element, out, displayStyle, duration, onComplete) => {
	...
	const interval = window.setInterval(() => {
		...
		} else opacity = (1 - startOpacity) * t + startOpacity;
		if (opacity > 0) {
...
var LoadingSpinner = class LoadingSpinner {
	...
	setMessageForTask(id, message) {
		for (let task of this.tasks) if (task.id === id) {
	...
	setContainer(container) {
		if (this.container && this.spinnerContainerOuter.parentElement === this.container) this.container.removeChild(this.spinnerContainerOuter);
		if (container) {
	...
	setMinimized(minimized, instant) {
		const showHideSpinner = (element, show, instant, displayStyle, fadeTransitionsIndex) => {
			if (instant) element.style.display = show ? displayStyle : "none";
			else this.fadeTransitions[fadeTransitionsIndex] = fadeElement(element, !show, displayStyle, STANDARD_FADE_DURATION, () => {
...
var LoadingProgressBar = class {
	...
	setContainer(container) {
		if (this.container && this.progressBarContainerOuter.parentElement === this.container) this.container.removeChild(this.progressBarContainerOuter);
		if (container) {
			this.container = container;
			this.container.appendChild(this.progressBarContainerOuter);
...
var InfoPanel = class {
	...
	update = function(renderDimensions, cameraPosition, cameraLookAtPosition, cameraUp, orthographicCamera, meshCursorPosition, currentFPS, splatCount, splatRenderCount, splatRenderCountPct, lastSortTime, focalAdjustment, splatScale, pointCloudMode) {
		const cameraPosString = `${cameraPosition.x.toFixed(5)}, ${cameraPosition.y.toFixed(5)}, ${cameraPosition.z.toFixed(5)}`;
		if (this.infoCells.cameraPosition.innerHTML !== cameraPosString) this.infoCells.cameraPosition.innerHTML = cameraPosString;
		if (cameraLookAtPosition) {
			...
			if (this.infoCells.cameraLookAt.innerHTML !== cameraLookAtString) this.infoCells.cameraLookAt.innerHTML = cameraLookAtString;
		}
		const cameraUpString = `${cameraUp.x.toFixed(5)}, ${cameraUp.y.toFixed(5)}, ${cameraUp.z.toFixed(5)}`;
		if (this.infoCells.cameraUp.innerHTML !== cameraUpString) this.infoCells.cameraUp.innerHTML = cameraUpString;
	...
	setContainer(container) {
		if (this.container && this.infoPanelContainer.parentElement === this.container) this.container.removeChild(this.infoPanelContainer);
		if (container) {
			this.container = container;
			this.container.appendChild(this.infoPanelContainer);
...
var ArrowHelper = class extends Object3D {
	constructor(dir = new Vector3(0, 0, 1), origin = new Vector3(0, 0, 0), length = 1, radius = .1, color = 16776960, headLength = length * .2, headRadius = headLength * .2) {
		super();
		...
		this.line = new Mesh(lineGeometry, new MeshBasicMaterial({
			color,
			toneMapped: false
		}));
...
var SceneHelper = class SceneHelper {
	...
	destroySplatRendertarget() {
		if (this.splatRenderTarget) this.splatRenderTarget = null;
	}
	...
	setupMeshCursor() {
		if (!this.meshCursor) {
			const coneGeometry = new ConeGeometry(.5, 1.5, 32);
	...
	setupControlPlane() {
		if (!this.controlPlane) {
			...
			planeMaterial.opacity = .6;
			planeMaterial.depthTest = false;
			planeMaterial.depthWrite = false;
			planeMaterial.side = 2;
			const planeMesh = new Mesh(planeGeometry, planeMaterial);
			const arrowDir = new Vector3(0, 1, 0);
			arrowDir.normalize();
			const arrowHelper = new ArrowHelper(arrowDir, new Vector3(0, 0, 0), .5, .01, 56576, .1, .03);
	...
	destroyDebugMeshes() {
		for (let debugRoot of [this.debugRoot, this.secondaryDebugRoot]) if (debugRoot) {
...
var Ray = class {
	...
	intersectSphere = function() {
		...
		return function(center, radius, outHit) {
			...
			const diffSq = toSphereCenterVec.dot(toSphereCenterVec) - toClosestApproachSq;
...
var Raycaster = class {
	...
	setFromCameraAndScreenPosition = function() {
		...
		return function(camera, screenPosition, screenDimensions) {
			...
			if (camera.isPerspectiveCamera) {
				...
				this.ray.direction.set(ndcCoords.x, ndcCoords.y, .5).unproject(camera).sub(this.ray.origin).normalize();
				this.camera = camera;
	...
	intersectSplatMesh = function() {
		...
		return function(splatMesh, outHits = []) {
			...
			for (let s = 0; s < splatTree.subTrees.length; s++) {
				...
				if (subTree.rootNode) this.castRayAtSplatTreeNode(localRay, splatTree, subTree.rootNode, outHitsForSubTree);
	...
	castRayAtSplatTreeNode = function() {
		...
		return function(ray, splatTree, node, outHits = []) {
			...
			if (node.data && node.data.indexes && node.data.indexes.length > 0) for (let i = 0; i < node.data.indexes.length; i++) {
				const splatGlobalIndex = node.data.indexes[i];
				const splatSceneIndex = splatTree.splatMesh.getSceneIndexForSplat(splatGlobalIndex);
				if (!splatTree.splatMesh.getScene(splatSceneIndex).visible) continue;
				...
				if (tempScale.x <= scaleEpsilon || tempScale.y <= scaleEpsilon || splatTree.splatMesh.splatRenderMode === SplatRenderMode.ThreeD && tempScale.z <= scaleEpsilon) continue;
			...
			if (node.children && node.children.length > 0) for (let child of node.children) this.castRayAtSplatTreeNode(ray, splatTree, child, outHits);
			return outHits;
		};
	}();
};
var SplatMaterial = class {
    ...
    `;
		if (enableOptionalEffects) vertexShaderSource += `
		...
		if (dynamicMode) vertexShaderSource += `
            ...
            `;
		else vertexShaderSource += `mat4 transformModelViewMatrix = modelViewMatrix;`;
		vertexShaderSource += `
		...
		if (maxSphericalHarmonicsDegree >= 1) {
			...
			if (maxSphericalHarmonicsDegree === 1) vertexShaderSource += `
                ...
                `;
			else if (maxSphericalHarmonicsDegree === 2) vertexShaderSource += `
                    vec4 sampledSH0123;
                    vec4 sampledSH4567;
	...
	static getUniforms(dynamicMode = false, enableOptionalEffects = false, maxSphericalHarmonicsDegree = 0, splatScale = 1, pointCloudModeEnabled = false) {
		...
		if (enableOptionalEffects) {
			const sceneOpacity = [];
			for (let i = 0; i < Constants.MaxScenes; i++) sceneOpacity.push(1);
			uniforms["sceneOpacity"] = {
				"type": "f",
				"value": sceneOpacity
			};
			const sceneVisibility = [];
			for (let i = 0; i < Constants.MaxScenes; i++) sceneVisibility.push(1);
		...
		if (dynamicMode) {
			...
			for (let i = 0; i < Constants.MaxScenes; i++) transformMatrices.push(new Matrix4());
			uniforms["transforms"] = {
				"type": "mat4",
...
var SplatMaterial3D = class SplatMaterial3D {
	/**
	...
	*/
	static build(dynamicMode = false, enableOptionalEffects = false, antialiased = false, maxScreenSpaceSplatSize = 2048, splatScale = 1, pointCloudModeEnabled = false, maxSphericalHarmonicsDegree = 0, kernel2DSize = .3) {
		...
		const uniforms = SplatMaterial.getUniforms(dynamicMode, enableOptionalEffects, maxSphericalHarmonicsDegree, splatScale, pointCloudModeEnabled);
...
var SplatMaterial2D = class SplatMaterial2D {
	/**
	* Build the Three.js material that is used to render the splats.
	* @param {number} dynamicMode If true, it means the scene geometry represented by this splat mesh is not stationary or
	*                             that the splat count might change
...
var SplatGeometry = class {
	...
	*/
	static build(maxSplatCount) {
		...
		const positions = new BufferAttribute(/* @__PURE__ */ new Float32Array(12), 3);
...
var SplatTreeNode = class SplatTreeNode {
	static idGen = 0;
	constructor(min, max, depth, id) {
		this.min = new Vector3().copy(min);
		this.max = new Vector3().copy(max);
		this.boundingBox = new Box3(this.min, this.max);
		this.center = new Vector3().copy(this.max).sub(this.min).multiplyScalar(.5).add(this.min);
		this.depth = depth;
		this.children = [];
		this.data = null;
...
var SplatSubTree = class SplatSubTree {
	...
	static convertWorkerSubTreeNode(workerSubTreeNode) {
		const convertedNode = new SplatTreeNode(new Vector3().fromArray(workerSubTreeNode.min), new Vector3().fromArray(workerSubTreeNode.max), workerSubTreeNode.depth, workerSubTreeNode.id);
		if (workerSubTreeNode.data.indexes) {
			convertedNode.data = { "indexes": [] };
			for (let index of workerSubTreeNode.data.indexes) convertedNode.data.indexes.push(index);
		}
		if (workerSubTreeNode.children) for (let child of workerSubTreeNode.children) convertedNode.children.push(SplatSubTree.convertWorkerSubTreeNode(child));
	...
	static convertWorkerSubTree(workerSubTree, splatMesh) {
		...
		const visitLeavesFromNode = (node, visitFunc) => {
			...
			for (let child of node.children) visitLeavesFromNode(child, visitFunc);
		};
		convertedSubTree.nodesWithIndexes = [];
		visitLeavesFromNode(convertedSubTree.rootNode, (node) => {
			if (node.data && node.data.indexes && node.data.indexes.length > 0) convertedSubTree.nodesWithIndexes.push(node);
		});
...
function createSplatTreeWorker(self) {
	...
	processSplatTreeNode = function(tree, node, indexToCenter, sceneCenters) {
		...
		if (splatCount < tree.maxCentersPerNode || node.depth > tree.maxDepth) {
			const newIndexes = [];
			for (let i = 0; i < node.data.indexes.length; i++) if (!tree.addedIndexes[node.data.indexes[i]]) {
		...
		for (let i = 0; i < splatCount; i++) {
			...
			for (let j = 0; j < childrenBounds.length; j++) if (childrenBounds[j].containsPoint(center)) {
				splatCounts[j]++;
		...
		for (let i = 0; i < childrenBounds.length; i++) {
			...
			childNode.data = { "indexes": baseIndexes[i] };
			node.children.push(childNode);
		}
		node.data = {};
		for (let child of node.children) processSplatTreeNode(tree, child, indexToCenter, sceneCenters);
	};
	const buildSubTree = (sceneCenters, maxDepth, maxCentersPerNode) => {
		...
		subTree.rootNode.data = { "indexes": indexes };
		return subTree;
	};
	function createSplatTree(allCenters, maxDepth, maxCentersPerNode) {
		...
		self.postMessage({ "subTrees": subTrees });
	}
	self.onmessage = (e) => {
		if (e.data.process) createSplatTree(e.data.process.centers, e.data.process.maxDepth, e.data.process.maxCentersPerNode);
	};
}
function workerProcessCenters(splatTreeWorker, centers, transferBuffers, maxDepth, maxCentersPerNode) {
	splatTreeWorker.postMessage({ "process": {
		"centers": centers,
		"maxDepth": maxDepth,
...
function checkAndCreateWorker() {
	return new Worker(URL.createObjectURL(new Blob([
		"(",
		createSplatTreeWorker.toString(),
		")(self)"
...
var SplatTree = class {
	...
	*/
			...
			delayedExecute(() => {
				...
				if (splatMesh.dynamicMode) {
					...
					for (let s = 0; s < splatMesh.scenes.length; s++) {
						const splatCount = splatMesh.getScene(s).splatBuffer.getSplatCount();
	...
	visitLeaves(visitFunc) {
		...
		for (let subTree of this.subTrees) visitLeavesFromNode(subTree.rootNode, visitFunc);
	}
};
...
var SCENE_FADEIN_RATE_FAST = .012;
var SCENE_FADEIN_RATE_GRADUAL = .003;
...
var SplatMesh = class SplatMesh extends Mesh {
	...
	*/
	static buildScenes(parentObject, splatBuffers, sceneOptions) {
		...
		for (let i = 0; i < splatBuffers.length; i++) {
			...
			let positionArray = options["position"] || [
				0,
				0,
				0
			];
			let rotationArray = options["rotation"] || [
				0,
				0,
				0,
				1
			];
			let scaleArray = options["scale"] || [
				1,
				1,
				1
			];
			...
			const scene = SplatMesh.createScene(splatBuffer, position, rotation, scale, options.splatAlphaRemovalThreshold || 1, options.opacity, options.visible);
	...
	*/
	static buildSplatIndexMaps(splatBuffers) {
		...
		for (let s = 0; s < splatBuffers.length; s++) {
			const maxSplatCount = splatBuffers[s].getMaxSplatCount();
	...
	buildSplatTree = function(minAlphas = [], onSplatTreeIndexesUpload, onSplatTreeConstruction) {
		return new Promise((resolve) => {
			...
			this.baseSplatTree.processSplatMesh(this, (splatIndex) => {
				...
				const minAlpha = minAlphas[this.getSceneIndexForSplat(splatIndex)] || 1;
	...
	*/
	build(splatBuffers, sceneOptions, keepSceneTransforms = true, finalBuild = false, onSplatTreeIndexesUpload, onSplatTreeConstruction, preserveVisibleRegion = true) {
		...
		if (keepSceneTransforms) for (let i = 0; i < this.scenes.length && i < newScenes.length; i++) {
		...
		for (let splatBuffer of splatBuffers) {
			...
			if (splatBufferSphericalHarmonicsDegree < minSphericalHarmonicsDegree) minSphericalHarmonicsDegree = splatBufferSphericalHarmonicsDegree;
		}
		...
		if (splatBuffers.length !== this.lastBuildScenes.length) splatBuffersChanged = true;
		else for (let i = 0; i < splatBuffers.length; i++) if (splatBuffers[i] !== this.lastBuildScenes[i].splatBuffer) {
			splatBuffersChanged = true;
			break;
		}
		let isUpdateBuild = true;
		if (this.scenes.length !== 1 || this.lastBuildSceneCount !== this.scenes.length || this.lastBuildMaxSplatCount !== maxSplatCount || splatBuffersChanged) isUpdateBuild = false;
		if (!isUpdateBuild) {
			this.boundingBox = new Box3();
			...
			if (this.splatRenderMode === SplatRenderMode.ThreeD) this.material = SplatMaterial3D.build(this.dynamicMode, this.enableOptionalEffects, this.antialiased, this.maxScreenSpaceSplatSize, this.splatScale, this.pointCloudModeEnabled, this.minSphericalHarmonicsDegree, this.kernel2DSize);
			else this.material = SplatMaterial2D.build(this.dynamicMode, this.enableOptionalEffects, this.splatScale, this.pointCloudModeEnabled, this.minSphericalHarmonicsDegree);
		...
		for (let i = 0; i < this.scenes.length; i++) this.lastBuildScenes[i] = this.scenes[i];
		this.lastBuildSplatCount = splatBufferSplatCount;
		this.lastBuildMaxSplatCount = this.getMaxSplatCount();
		this.lastBuildSceneCount = this.scenes.length;
		if (finalBuild && this.scenes.length > 0) this.buildSplatTree(sceneOptions.map((options) => options.splatAlphaRemovalThreshold || 1), onSplatTreeIndexesUpload, onSplatTreeConstruction).then(() => {
	...
	freeIntermediateSplatData() {
		...
		if (this.splatDataTextures.sphericalHarmonics) delete this.splatDataTextures.sphericalHarmonics.data;
		if (this.splatDataTextures.sceneIndexes) delete this.splatDataTextures.sceneIndexes.data;
		...
		if (this.splatDataTextures.sphericalHarmonics) {
			...
			} else this.splatDataTextures.sphericalHarmonics.textures.forEach((texture) => {
	...
	*/
	...
	disposeTextures() {
		for (let textureKey in this.splatDataTextures) if (this.splatDataTextures.hasOwnProperty(textureKey)) {
	...
	*/
	getDataForDistancesComputation(start, end) {
		return {
			centers: this.integerBasedDistancesComputation ? this.getIntegerCenters(start, end, true) : this.getFloatCenters(start, end, true),
			sceneIndexes: this.getSceneIndexes(start, end)
		};
	}
	/**
	...
	*/
	refreshGPUDataFromSplatBuffers(sinceLastBuildOnly) {
		...
		if (this.enableDistancesComputationOnGPU) this.refreshGPUBuffersForDistancesComputation(centers, sceneIndexes, sinceLastBuildOnly);
		return {
			"from": updateStart,
			"to": splatCount - 1,
	...
	*/
	refreshDataTexturesFromSplatBuffers(sinceLastBuildOnly) {
		...
		} else this.updateBaseDataFromSplatBuffers(fromSplat, toSplat);
	...
	setupDataTextures() {
		...
		const getCovariancesInitialTextureSpecs = (compressionLevel) => {
			...
			return {
				...
				texSize: computeDataTextureSize(elementsPerTexelStored, 6)
			};
		};
		...
		if (this.splatRenderMode === SplatRenderMode.ThreeD) {
			...
			if (initialCovTexSpecs.texSize.x * initialCovTexSpecs.texSize.y > MAX_TEXTURE_TEXELS && covarianceCompressionLevel === 0) covarianceCompressionLevel = 1;
		...
		const centersColsTex = new DataTexture(paddedCentersCols, centersColsTexSize.x, centersColsTexSize.y, RGBAIntegerFormat, UnsignedIntType);
		...
		if (this.splatRenderMode === SplatRenderMode.ThreeD) {
			...
			else SplatMesh.updatePaddedCompressedCovariancesTextureData(covariances, covariancesTextureData, 0, 0, covariances.length);
			let covTex;
			if (covarianceCompressionLevel >= 1) {
				covTex = new DataTexture(covariancesTextureData, covTexSize.x, covTexSize.y, RGBAIntegerFormat, UnsignedIntType);
				...
				const dummyTex = new DataTexture(/* @__PURE__ */ new Uint32Array(32), 2, 2, RGBAIntegerFormat, UnsignedIntType);
			...
			const scaleRotationsTexSize = computeDataTextureSize(SCALES_ROTATIONS_ELEMENTS_PER_TEXEL, 6);
			...
			const scaleRotationsTex = new DataTexture(paddedScaleRotations, scaleRotationsTexSize.x, scaleRotationsTexSize.y, RGBAFormat, scaleRotationsTextureType);
		...
		if (shData) {
			...
			if (shTexSize.x * shTexSize.y <= MAX_TEXTURE_TEXELS) {
				...
				for (let c = 0; c < splatCount; c++) {
					...
					for (let i = 0; i < shComponentCount; i++) paddedSHArray[destBase + i] = shData[srcBase + i];
				}
				...
				for (let t = 0; t < 3; t++) {
					...
					for (let c = 0; c < splatCount; c++) {
						...
						if (shComponentCountPerChannel >= 3) {
							...
							if (shComponentCountPerChannel >= 8) for (let i = 0; i < 5; i++) paddedSHArray[destBase + 3 + i] = shData[srcBase + 9 + t * 5 + i];
						}
					}
					const shTexture = new DataTexture(paddedSHArray, shTexSize.x, shTexSize.y, texelFormat, shTextureType);
		...
		const sceneIndexesTexture = new DataTexture(paddedTransformIndexes, sceneIndexesTexSize.x, sceneIndexesTexSize.y, RedIntegerFormat, UnsignedIntType);
	...
	updateBaseDataFromSplatBuffers(fromSplat, toSplat) {
		...
		this.fillSplatDataArrays(this.splatDataTextures.baseData.covariances, this.splatDataTextures.baseData.scales, this.splatDataTextures.baseData.rotations, this.splatDataTextures.baseData.centers, this.splatDataTextures.baseData.colors, this.splatDataTextures.baseData.sphericalHarmonics, void 0, covarianceCompressionLevel, scaleRotationCompressionLevel, shCompressionLevel, fromSplat, toSplat, fromSplat);
	}
	updateDataTexturesFromBaseData(fromSplat, toSplat) {
		...
		SplatMesh.updateCenterColorsPaddedData(fromSplat, toSplat, this.splatDataTextures.baseData.centers, this.splatDataTextures.baseData.colors, paddedCenterColors);
		const centerColorsTextureProps = this.renderer ? this.renderer.properties.get(centerColorsTexture) : null;
		if (!centerColorsTextureProps || !centerColorsTextureProps.__webglTexture) centerColorsTexture.needsUpdate = true;
		else this.updateDataTexture(paddedCenterColors, centerColorsTextureDescriptor.texture, centerColorsTextureDescriptor.size, centerColorsTextureProps, CENTER_COLORS_ELEMENTS_PER_TEXEL, CENTER_COLORS_ELEMENTS_PER_SPLAT, 4, fromSplat, toSplat);
		if (covarancesTextureDesc) {
			...
			if (covarianceCompressionLevel === 0) for (let i = covarancesStartElement; i <= covariancesEndElement; i++) {
				const covariance = this.splatDataTextures.baseData.covariances[i];
				covarancesTextureDesc.data[i] = covariance;
			}
			else SplatMesh.updatePaddedCompressedCovariancesTextureData(this.splatDataTextures.baseData.covariances, covarancesTextureDesc.data, fromSplat * covarancesTextureDesc.elementsPerTexelAllocated, covarancesStartElement, covariancesEndElement);
			const covariancesTextureProps = this.renderer ? this.renderer.properties.get(covariancesTexture) : null;
			if (!covariancesTextureProps || !covariancesTextureProps.__webglTexture) covariancesTexture.needsUpdate = true;
			else if (covarianceCompressionLevel === 0) this.updateDataTexture(covarancesTextureDesc.data, covarancesTextureDesc.texture, covarancesTextureDesc.size, covariancesTextureProps, covarancesTextureDesc.elementsPerTexelStored, COVARIANCES_ELEMENTS_PER_SPLAT, 4, fromSplat, toSplat);
			else this.updateDataTexture(covarancesTextureDesc.data, covarancesTextureDesc.texture, covarancesTextureDesc.size, covariancesTextureProps, covarancesTextureDesc.elementsPerTexelAllocated, covarancesTextureDesc.elementsPerTexelAllocated, 2, fromSplat, toSplat);
		}
		if (scaleRotationsTextureDesc) {
			...
			SplatMesh.updateScaleRotationsPaddedData(fromSplat, toSplat, this.splatDataTextures.baseData.scales, this.splatDataTextures.baseData.rotations, paddedScaleRotations);
			const scaleRotationsTextureProps = this.renderer ? this.renderer.properties.get(scaleRotationsTexture) : null;
			if (!scaleRotationsTextureProps || !scaleRotationsTextureProps.__webglTexture) scaleRotationsTexture.needsUpdate = true;
			else this.updateDataTexture(paddedScaleRotations, scaleRotationsTextureDesc.texture, scaleRotationsTextureDesc.size, scaleRotationsTextureProps, SCALES_ROTATIONS_ELEMENTS_PER_TEXEL, elementsPerSplat, bytesPerElement, fromSplat, toSplat);
		}
		...
		if (shData) {
			...
			const updateTexture = (shTexture, shTextureSize, elementsPerTexel, paddedSHArray, paddedSHComponentCount) => {
				...
				if (!shTextureProps || !shTextureProps.__webglTexture) shTexture.needsUpdate = true;
				else this.updateDataTexture(paddedSHArray, shTexture, shTextureSize, shTextureProps, elementsPerTexel, paddedSHComponentCount, shBytesPerElement, fromSplat, toSplat);
			};
			...
			if (shTextureDesc.textureCount === 1) {
				...
				updateTexture(shTextureDesc.texture, shTextureDesc.size, shTextureDesc.elementsPerTexel, paddedSHArray, paddedSHComponentCount);
			} else {
				...
				for (let t = 0; t < 3; t++) {
					...
					updateTexture(shTextureDesc.textures[t], shTextureDesc.size, shTextureDesc.elementsPerTexel, paddedSHArray, paddedSHComponentCount);
				}
			}
		}
		const sceneIndexesTexDesc = this.splatDataTextures["sceneIndexes"];
		const paddedSceneIndexes = sceneIndexesTexDesc.data;
		for (let c = this.lastBuildSplatCount; c <= toSplat; c++) paddedSceneIndexes[c] = this.globalSplatIndexToSceneIndexMap[c];
		...
		if (!sceneIndexesTextureProps || !sceneIndexesTextureProps.__webglTexture) sceneIndexesTexture.needsUpdate = true;
		else this.updateDataTexture(paddedSceneIndexes, sceneIndexesTexDesc.texture, sceneIndexesTexDesc.size, sceneIndexesTextureProps, 1, 1, 1, this.lastBuildSplatCount, toSplat);
	}
	...
	getMaximumSplatBufferCompressionLevel() {
		...
		for (let i = 0; i < this.scenes.length; i++) {
			const splatBuffer = this.getScene(i).splatBuffer;
			if (i === 0 || splatBuffer.compressionLevel > maxCompressionLevel) maxCompressionLevel = splatBuffer.compressionLevel;
		}
	...
	getMinimumSplatBufferCompressionLevel() {
		...
		for (let i = 0; i < this.scenes.length; i++) {
			const splatBuffer = this.getScene(i).splatBuffer;
			if (i === 0 || splatBuffer.compressionLevel < minCompressionLevel) minCompressionLevel = splatBuffer.compressionLevel;
		}
	...
	static computeTextureUpdateRegion(startSplat, endSplat, textureWidth, elementsPerTexel, elementsPerSplat) {
		...
		return {
			...
			"dataEnd": endRow * textureWidth * elementsPerTexel + textureWidth * elementsPerTexel,
			"startRow": startRow,
			"endRow": endRow
		};
	}
	updateDataTexture(paddedData, texture, textureSize, textureProps, elementsPerTexel, elementsPerSplat, bytesPerElement, from, to) {
		...
		const updateDataView = new paddedData.constructor(paddedData.buffer, updateRegion.dataStart * bytesPerElement, updateElementCount);
		...
		gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, updateRegion.startRow, textureSize.x, updateHeight, glFormat, glType, updateDataView);
	...
	updateVisibleRegionFadeDistance(sceneRevealMode = SceneRevealMode.Default) {
		...
		const fadeInComplete = (this.visibleRegionBufferRadius > 0 ? this.visibleRegionFadeStartRadius / this.visibleRegionBufferRadius : 0) > .99;
	...
	*/
	updateTransforms() {
		for (let i = 0; i < this.scenes.length; i++) this.getScene(i).updateTransform(this.dynamicMode);
	}
	updateUniforms = function() {
		...
		return function(renderDimensions, cameraFocalLengthX, cameraFocalLengthY, orthographicMode, orthographicZoom, inverseFocalAdjustment) {
			if (this.getSplatCount() > 0) {
				...
				if (this.dynamicMode) for (let i = 0; i < this.scenes.length; i++) this.material.uniforms.transforms.value[i].copy(this.getScene(i).transform);
				if (this.enableOptionalEffects) for (let i = 0; i < this.scenes.length; i++) {
	...
	static getTotalSplatCountForScenes(scenes) {
		let totalSplatCount = 0;
		for (let scene of scenes) if (scene && scene.splatBuffer) totalSplatCount += scene.splatBuffer.getSplatCount();
	...
	static getTotalMaxSplatCountForScenes(scenes) {
		let totalSplatCount = 0;
		for (let scene of scenes) if (scene && scene.splatBuffer) totalSplatCount += scene.splatBuffer.getMaxSplatCount();
		return totalSplatCount;
	}
	...
	*/
	...
	setupDistancesComputationTransformFeedback = function() {
		...
		return function() {
			...
			if (rebuildGPUObjects) this.disposeDistancesComputationGPUResources();
			else if (rebuildBuffers) this.disposeDistancesComputationGPUBufferResources();
			const gl = this.renderer.getContext();
			...
			if (rebuildGPUObjects) this.distancesTransformFeedback.vao = gl.createVertexArray();
			gl.bindVertexArray(this.distancesTransformFeedback.vao);
			if (rebuildGPUObjects) {
				const program = gl.createProgram();
				const vertexShader = createShader(gl, gl.VERTEX_SHADER, vsSource);
				const fragmentShader = createShader(gl, gl.FRAGMENT_SHADER, fsSource);
				if (!vertexShader || !fragmentShader) throw new Error("Could not compile shaders for distances computation on GPU.");
			...
			if (this.dynamicMode) {
				...
				for (let i = 0; i < this.scenes.length; i++) this.distancesTransformFeedback.transformsLocs[i] = gl.getUniformLocation(this.distancesTransformFeedback.program, `transforms[${i}]`);
			} else this.distancesTransformFeedback.modelViewProjLoc = gl.getUniformLocation(this.distancesTransformFeedback.program, "modelViewProj");
			if (rebuildGPUObjects || rebuildBuffers) {
				this.distancesTransformFeedback.centersBuffer = gl.createBuffer();
				gl.bindBuffer(gl.ARRAY_BUFFER, this.distancesTransformFeedback.centersBuffer);
				gl.enableVertexAttribArray(this.distancesTransformFeedback.centersLoc);
				if (this.integerBasedDistancesComputation) gl.vertexAttribIPointer(this.distancesTransformFeedback.centersLoc, 4, gl.INT, 0, 0);
				else gl.vertexAttribPointer(this.distancesTransformFeedback.centersLoc, 4, gl.FLOAT, false, 0, 0);
				if (this.dynamicMode) {
			...
			if (rebuildGPUObjects || rebuildBuffers) this.distancesTransformFeedback.outDistancesBuffer = gl.createBuffer();
			gl.bindBuffer(gl.ARRAY_BUFFER, this.distancesTransformFeedback.outDistancesBuffer);
			gl.bufferData(gl.ARRAY_BUFFER, maxSplatCount * 4, gl.STATIC_READ);
			if (rebuildGPUObjects) this.distancesTransformFeedback.id = gl.createTransformFeedback();
	...
	*/
	updateGPUCentersBufferForDistancesComputation(isUpdate, centers, offsetSplats) {
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, this.distancesTransformFeedback.centersBuffer);
		if (isUpdate) gl.bufferSubData(gl.ARRAY_BUFFER, subBufferOffset, centers);
		else {
	...
	*/
	updateGPUTransformIndexesBufferForDistancesComputation(isUpdate, sceneIndexes, offsetSplats) {
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, this.distancesTransformFeedback.sceneIndexesBuffer);
		if (isUpdate) gl.bufferSubData(gl.ARRAY_BUFFER, subBufferOffset, sceneIndexes);
		else {
	...
	*/
	getSceneIndexes(start, end) {
		...
		for (let i = start; i <= end; i++) sceneIndexes[i] = this.globalSplatIndexToSceneIndexMap[i];
		return sceneIndexes;
	}
	/**
	...
	*/
	fillTransformsArray = function() {
		...
		return function(array) {
			...
			for (let i = 0; i < this.scenes.length; i++) {
				const sceneTransformElements = this.getScene(i).transform.elements;
				for (let j = 0; j < 16; j++) tempArray[i * 16 + j] = sceneTransformElements[j];
			}
			array.set(tempArray);
		};
	}();
	computeDistancesOnGPU = function() {
		...
		return function(modelViewProjMatrix, outComputedDistances) {
			...
			if (this.dynamicMode) for (let i = 0; i < this.scenes.length; i++) {
				...
				if (this.integerBasedDistancesComputation) {
					...
					gl.uniform4i(this.distancesTransformFeedback.transformsLocs[i], iTransform[0], iTransform[1], iTransform[2], iTransform[3]);
				} else gl.uniformMatrix4fv(this.distancesTransformFeedback.transformsLocs[i], false, tempMatrix.elements);
			}
	...
	*/
	getLocalSplatParameters(globalIndex, paramsObj, returnSceneTransform) {
		if (returnSceneTransform === void 0 || returnSceneTransform === null) returnSceneTransform = this.dynamicMode ? false : true;
	...
	*/
	fillSplatDataArrays(covariances, scales, rotations, centers, colors, sphericalHarmonics, applySceneTransform, covarianceCompressionLevel = 0, scaleRotationCompressionLevel = 0, sphericalHarmonicsCompressionLevel = 1, srcStart, srcEnd, destStart = 0, sceneIndex) {
		const scaleOverride = new Vector3();
		scaleOverride.x = void 0;
		scaleOverride.y = void 0;
		if (this.splatRenderMode === SplatRenderMode.ThreeD) scaleOverride.z = void 0;
		else scaleOverride.z = 1;
		const tempTransform = new Matrix4();
		let startSceneIndex = 0;
		let endSceneIndex = this.scenes.length - 1;
		if (sceneIndex !== void 0 && sceneIndex !== null && sceneIndex >= 0 && sceneIndex <= this.scenes.length) {
			startSceneIndex = sceneIndex;
			endSceneIndex = sceneIndex;
		}
		for (let i = startSceneIndex; i <= endSceneIndex; i++) {
			if (applySceneTransform === void 0 || applySceneTransform === null) applySceneTransform = this.dynamicMode ? false : true;
			...
			if (covariances) splatBuffer.fillSplatCovarianceArray(covariances, sceneTransform, srcStart, srcEnd, destStart, covarianceCompressionLevel);
			if (scales || rotations) {
				if (!scales || !rotations) throw new Error("SplatMesh::fillSplatDataArrays() -> \"scales\" and \"rotations\" must both be valid.");
				splatBuffer.fillSplatScaleRotationArray(scales, rotations, sceneTransform, srcStart, srcEnd, destStart, scaleRotationCompressionLevel, scaleOverride);
			}
			...
			if (sphericalHarmonics) splatBuffer.fillSphericalHarmonicsArray(sphericalHarmonics, this.minSphericalHarmonicsDegree, sceneTransform, srcStart, srcEnd, destStart, sphericalHarmonicsCompressionLevel);
	...
	*/
	getIntegerCenters(start, end, padFour = false) {
		...
		for (let i = 0; i < splatCount; i++) {
			for (let t = 0; t < 3; t++) intCenters[i * componentCount + t] = Math.round(floatCenters[i * 3 + t] * 1e3);
	...
	*/
	getFloatCenters(start, end, padFour = false) {
		...
		for (let i = 0; i < splatCount; i++) {
			for (let t = 0; t < 3; t++) paddedFloatCenters[i * 4 + t] = floatCenters[i * 3 + t];
	...
	*/
	getSplatScaleAndRotation = function() {
		...
		return function(globalIndex, outScale, outRotation, applySceneTransform) {
			...
			paramsObj.splatBuffer.getSplatScaleAndRotation(paramsObj.localIndex, outScale, outRotation, paramsObj.sceneTransform, scaleOverride);
		};
	}();
	/**
	...
	*/
	getScene(sceneIndex) {
		if (sceneIndex < 0 || sceneIndex >= this.scenes.length) throw new Error("SplatMesh::getScene() -> Invalid scene index.");
	...
	static getIntegerMatrixArray(matrix) {
		...
		for (let i = 0; i < 16; i++) intMatrixArray[i] = Math.round(matrixElements[i] * 1e3);
		return intMatrixArray;
	}
	computeBoundingBox(applySceneTransforms = false, sceneIndex) {
		let splatCount = this.getSplatCount();
		if (sceneIndex !== void 0 && sceneIndex !== null) {
			if (sceneIndex < 0 || sceneIndex >= this.scenes.length) throw new Error("SplatMesh::computeBoundingBox() -> Invalid scene index.");
			splatCount = this.scenes[sceneIndex].splatBuffer.getSplatCount();
		}
		const floatCenters = new Float32Array(splatCount * 3);
		this.fillSplatDataArrays(null, null, null, floatCenters, null, null, applySceneTransforms, void 0, void 0, void 0, void 0, sceneIndex);
		const min = new Vector3();
		const max = new Vector3();
		for (let i = 0; i < splatCount; i++) {
			const offset = i * 3;
...
function sortWorker(self) {
	...
	function sort(splatSortCount, splatRenderCount, modelViewProj, usePrecomputedDistances, copyIndexesToSort, copyPrecomputedDistances, copyTransforms) {
		...
		if (!useSharedMemory) {
			new Uint32Array(wasmMemory, indexesToSortOffset, copyIndexesToSort.byteLength / Constants.BytesPerInt).set(copyIndexesToSort);
			new Float32Array(wasmMemory, transformsOffset, copyTransforms.byteLength / Constants.BytesPerFloat).set(copyTransforms);
			if (usePrecomputedDistances) {
				let precomputedDistances;
				if (integerBasedSort) precomputedDistances = new Int32Array(wasmMemory, precomputedDistancesOffset, copyPrecomputedDistances.byteLength / Constants.BytesPerInt);
				else precomputedDistances = new Float32Array(wasmMemory, precomputedDistancesOffset, copyPrecomputedDistances.byteLength / Constants.BytesPerFloat);
		...
		wasmInstance.exports.sortIndexes(indexesToSortOffset, centersOffset, precomputedDistancesOffset, mappedDistancesOffset, frequenciesOffset, modelViewProjOffset, sortedIndexesOffset, sceneIndexesOffset, transformsOffset, distanceMapRange, splatSortCount, splatRenderCount, splatCount, usePrecomputedDistances, integerBasedSort, dynamicMode);
		const sortMessage = {
			"sortDone": true,
		...
		if (!useSharedMemory) {
			...
			if (!sortedIndexesOut || sortedIndexesOut.length < splatRenderCount) sortedIndexesOut = new Uint32Array(splatRenderCount);
			sortedIndexesOut.set(sortedIndexes);
			sortMessage.sortedIndexes = sortedIndexesOut;
		}
		sortMessage.sortTime = performance.now() - sortStartTime;
	...
	self.onmessage = (e) => {
		if (e.data.centers) {
			...
			if (integerBasedSort) new Int32Array(wasmMemory, centersOffset + e.data.range.from * Constants.BytesPerInt * 4, e.data.range.count * 4).set(new Int32Array(centers));
			else new Float32Array(wasmMemory, centersOffset + e.data.range.from * Constants.BytesPerFloat * 4, e.data.range.count * 4).set(new Float32Array(centers));
			if (dynamicMode) new Uint32Array(wasmMemory, sceneIndexesOffset + e.data.range.from * 4, e.data.range.count).set(new Uint32Array(sceneIndexes));
			...
			sort(sortCount, renderCount, e.data.sort.modelViewProj, usePrecomputedDistances, copyIndexesToSort, copyPrecomputedDistances, copyTransforms);
		} else if (e.data.init) {
			...
			const memoryRequiredForPrecomputedDistances = integerBasedSort ? splatCount * Constants.BytesPerInt : splatCount * Constants.BytesPerFloat;
			const memoryRequiredForMappedDistances = splatCount * Constants.BytesPerInt;
			const memoryRequiredForSortedIndexes = splatCount * Constants.BytesPerInt;
			const memoryRequiredForIntermediateSortBuffers = integerBasedSort ? distanceMapRange * Constants.BytesPerInt * 2 : distanceMapRange * Constants.BytesPerFloat * 2;
			...
			WebAssembly.compile(sorterWasmBytes).then((wasmModule) => {
				...
				if (useSharedMemory) self.postMessage({
				...
				else self.postMessage({ "sortSetupPhase1Complete": true });
			});
		}
	};
}
function createSortWorker(splatCount, useSharedMemory, enableSIMDInSort, integerBasedSort, dynamicMode, splatSortDistanceMapPrecision = Constants.DefaultSplatSortDistanceMapPrecision) {
	...
	if (!enableSIMDInSort && !useSharedMemory) {
		...
		if (iOSSemVer && iOSSemVer.major <= 16 && iOSSemVer.minor < 4) sourceWasm = SorterWasmNoSIMDNonShared;
	} else if (!enableSIMDInSort) sourceWasm = SorterWasmNoSIMD;
	else if (!useSharedMemory) {
		if (iOSSemVer && iOSSemVer.major <= 16 && iOSSemVer.minor < 4) sourceWasm = SorterWasmNonShared;
	}
	const sorterWasmBinaryString = atob(sourceWasm);
	const sorterWasmBytes = new Uint8Array(sorterWasmBinaryString.length);
	for (let i = 0; i < sorterWasmBinaryString.length; i++) sorterWasmBytes[i] = sorterWasmBinaryString.charCodeAt(i);
		...
		function showEnterVR() {
			...
			button.onclick = function() {
				if (currentSession === null) navigator.xr.requestSession("immersive-vr", sessionOptions).then(onSessionStarted);
					...
					currentSession.end();
					if (navigator.xr.offerSession !== void 0) navigator.xr.offerSession("immersive-vr", sessionOptions).then(onSessionStarted).catch((err) => {
						console.warn(err);
					});
				}
			};
	...
	static createButton(renderer, sessionInit = {}) {
		...
		function showStartAR() {
			if (sessionInit.domOverlay === void 0) {
				...
				if (sessionInit.optionalFeatures === void 0) sessionInit.optionalFeatures = [];
			...
			button.onclick = function() {
				if (currentSession === null) navigator.xr.requestSession("immersive-ar", sessionInit).then(onSessionStarted);
					...
					currentSession.end();
					if (navigator.xr.offerSession !== void 0) navigator.xr.offerSession("immersive-ar", sessionInit).then(onSessionStarted).catch((err) => {
						console.warn(err);
					});
				}
			};
...
var MINIMUM_DISTANCE_TO_NEW_FOCAL_POINT = .75;
...
var Viewer = class Viewer {
	constructor(options = {}) {
		if (!options.cameraUp) options.cameraUp = [
			0,
			1,
			0
		];
		this.cameraUp = new Vector3().fromArray(options.cameraUp);
		if (!options.initialCameraPosition) options.initialCameraPosition = [
			0,
			10,
			15
		];
		this.initialCameraPosition = new Vector3().fromArray(options.initialCameraPosition);
		if (!options.initialCameraLookAt) options.initialCameraLookAt = [
			0,
			0,
			0
		];
		...
		if (options.integerBasedSort === void 0 || options.integerBasedSort === null) options.integerBasedSort = true;
		...
		this.kernel2DSize = options.kernel2DSize === void 0 ? .3 : options.kernel2DSize;
		this.webXRMode = options.webXRMode || WebXRMode.None;
		if (this.webXRMode !== WebXRMode.None) this.gpuAcceleratedSort = false;
		...
		if (options.inMemoryCompressionLevel === void 0 || options.inMemoryCompressionLevel === null) options.inMemoryCompressionLevel = 0;
		this.inMemoryCompressionLevel = options.inMemoryCompressionLevel;
		if (options.optimizeSplatData === void 0 || options.optimizeSplatData === null) options.optimizeSplatData = true;
		this.optimizeSplatData = options.optimizeSplatData;
		if (options.freeIntermediateSplatData === void 0 || options.freeIntermediateSplatData === null) options.freeIntermediateSplatData = false;
		...
		if (options.splatRenderMode === void 0 || options.splatRenderMode === null) options.splatRenderMode = SplatRenderMode.ThreeD;
		this.splatRenderMode = options.splatRenderMode;
		this.sceneFadeInRateMultiplier = options.sceneFadeInRateMultiplier || 1;
	...
	init() {
		...
		if (!this.rootElement) {
			...
			} else this.rootElement = this.renderer.domElement || document.body;
		}
		this.setupCamera();
		this.setupRenderer();
	...
	setupCamera() {
		if (!this.usingExternalCamera) {
			...
			this.perspectiveCamera = new PerspectiveCamera(THREE_CAMERA_FOV, renderDimensions.x / renderDimensions.y, .1, 1e3);
			this.orthographicCamera = new OrthographicCamera(renderDimensions.x / -2, renderDimensions.x / 2, renderDimensions.y / 2, renderDimensions.y / -2, .1, 1e3);
	...
	setupWebXR(webXRSessionInit) {
		if (this.webXRMode) {
			if (this.webXRMode === WebXRMode.VR) this.rootElement.appendChild(VRButton.createButton(this.renderer, webXRSessionInit));
			else if (this.webXRMode === WebXRMode.AR) this.rootElement.appendChild(ARButton.createButton(this.renderer, webXRSessionInit));
	...
	setupControls() {
		if (this.useBuiltInControls && this.webXRMode === WebXRMode.None) {
			...
			} else if (this.camera.isOrthographicCamera) this.orthographicControls = new OrbitControls(this.camera, this.renderer.domElement);
			else this.perspectiveControls = new OrbitControls(this.camera, this.renderer.domElement);
			for (let controls of [this.orthographicControls, this.perspectiveControls]) if (controls) {
		...
		return function(e) {
			...
			switch (e.code) {
				...
				case "KeyI":
					...
					break;
				case "KeyO":
					if (!this.usingExternalCamera) this.setOrthographicMode(!this.camera.isOrthographicCamera);
					break;
				case "KeyP":
					if (!this.usingExternalCamera) this.splatMesh.setPointCloudModeEnabled(!this.splatMesh.getPointCloudModeEnabled());
					break;
				case "Equal":
					if (!this.usingExternalCamera) this.splatMesh.setSplatScale(this.splatMesh.getSplatScale() + .05);
					break;
				case "Minus": if (!this.usingExternalCamera) this.splatMesh.setSplatScale(Math.max(this.splatMesh.getSplatScale() - .05, 0));
			}
		};
	}();
	onMouseMove(mouse) {
	...
	onMouseUp = function() {
		...
		return function(mouse) {
			...
			if (getCurrentTime() - this.mouseDownTime < .5 && clickOffset.length() < 2) this.onMouseClick(mouse);
		};
	}();
	onMouseClick(mouse) {
		this.mousePosition.set(mouse.offsetX, mouse.offsetY);
	...
	getRenderDimensions(outDimensions) {
		...
		} else this.renderer.getSize(outDimensions);
	}
	setOrthographicMode(orthographicMode) {
		...
		if (this.controls) {
			...
			if (orthographicMode) Viewer.setCameraZoomFromPosition(toCamera, fromCamera, fromControls);
			else Viewer.setCameraPositionFromZoom(toCamera, fromCamera, toControls);
	...
	static setCameraPositionFromZoom = function() {
		...
		return function(positionCamera, zoomedCamera, controls) {
			const toLookAtDistance = 1 / (zoomedCamera.zoom * .001);
	...
	static setCameraZoomFromPosition = function() {
		...
		return function(zoomCamera, positionZamera, controls) {
			zoomCamera.zoom = 1 / (tempVector.copy(controls.target).sub(positionZamera.position).length() * .001);
		};
	}();
	updateSplatMesh = function() {
		...
		return function() {
			...
			if (this.splatMesh.getSplatCount() > 0) {
				...
				this.getRenderDimensions(renderDimensions);
				const focalLengthX = this.camera.projectionMatrix.elements[0] * .5 * this.devicePixelRatio * renderDimensions.x;
				const focalLengthY = this.camera.projectionMatrix.elements[5] * .5 * this.devicePixelRatio * renderDimensions.y;
				...
				this.splatMesh.updateUniforms(renderDimensions, focalLengthX * focalAdjustment, focalLengthY * focalAdjustment, this.camera.isOrthographicCamera, this.camera.zoom || 1, inverseFocalAdjustment);
			}
		};
	}();
	adjustForWebXRStereo(renderDimensions) {
		if (this.camera && this.webXRActive) {
			const xrCameraProj00 = this.renderer.xr.getCamera().projectionMatrix.elements[0];
	...
	*/
	addSplatScene(path, options = {}) {
		if (this.isLoadingOrUnloading()) throw new Error("Cannot add splat scene while another load or unload is already in progress.");
		if (this.isDisposingOrDisposed()) throw new Error("Cannot add splat scene after dispose() is called.");
		...
		const onProgressUIUpdate = (percentComplete, percentCompleteLabel, loaderStatus) => {
			if (showLoadingUI) {
				if (loaderStatus === LoaderStatus.Downloading) {
					if (percentComplete == 100) this.loadingSpinner.setMessageForTask(loadingUITaskId, "Download complete!");
					else if (progressiveLoad) this.loadingSpinner.setMessageForTask(loadingUITaskId, "Downloading splats...");
					else {
				...
				} else if (loaderStatus === LoaderStatus.Processing) this.loadingSpinner.setMessageForTask(loadingUITaskId, "Processing splats...");
			}
		};
			...
			if (showLoadingUI) {
				...
				if (progressiveLoad) {
					if (finalBuild) {
						downloadDone = true;
						this.loadingProgressBar.hide();
					} else this.loadingProgressBar.setProgress(downloadedPercentage);
				}
			}
		};
		...
		const buildSection = (splatBuffer, firstBuild, finalBuild) => {
			...
			return this.addSplatBuffers([splatBuffer], [addSplatBufferOptions], finalBuild, firstBuild && showLoadingUI, showLoadingUI, progressiveLoad, progressiveLoad).then(() => {
		...
		return (progressiveLoad ? this.downloadAndBuildSingleSplatSceneProgressiveLoad.bind(this) : this.downloadAndBuildSingleSplatSceneStandardLoad.bind(this))(path, format, options.splatAlphaRemovalThreshold, buildSection.bind(this), onProgress, hideLoadingUI.bind(this), options.headers);
	}
	/**
	...
	*/
	downloadAndBuildSingleSplatSceneStandardLoad(path, format, splatAlphaRemovalThreshold, buildFunc, onProgress, onException, headers) {
		const downloadPromise = this.downloadSplatSceneToSplatBuffer(path, splatAlphaRemovalThreshold, onProgress, false, void 0, format, headers);
		...
		const checkAndBuildProgressiveLoadSections = () => {
			if (queuedProgressiveLoadSectionBuilds.length > 0 && !progressiveLoadedSectionBuilding && !this.isDisposingOrDisposed()) {
				...
				buildFunc(queuedBuild.splatBuffer, queuedBuild.firstBuild, queuedBuild.finalBuild).then(() => {
					...
					if (queuedBuild.firstBuild) progressiveLoadFirstSectionBuildPromise.resolve();
					else if (queuedBuild.finalBuild) {
						splatSceneDownloadAndBuildPromise.resolve();
						this.clearSplatSceneDownloadAndBuildPromise();
					}
					if (queuedProgressiveLoadSectionBuilds.length > 0) delayedExecute(() => checkAndBuildProgressiveLoadSections());
				});
			}
		};
	...
	*/
	addSplatScenes(sceneOptions, showLoadingUI = true, onProgress = void 0) {
		...
		const onLoadProgress = (fileIndex, percent, percentLabel, loaderStatus) => {
			...
			if (showLoadingUI) {
				if (loaderStatus === LoaderStatus.Downloading) this.loadingSpinner.setMessageForTask(loadingUITaskId, totalPercent == 100 ? `Download complete!` : `Downloading: ${percentLabel}`);
			}
		...
		for (let i = 0; i < sceneOptions.length; i++) {
			...
			const baseDownloadPromise = this.downloadSplatSceneToSplatBuffer(options.path, options.splatAlphaRemovalThreshold, onLoadProgress.bind(this, i), false, void 0, format, options.headers);
		...
		const downloadAndBuildPromise = new AbortablePromise((resolve, reject) => {
			...
			for (let baseDownloadPromise of baseDownloadPromises) baseDownloadPromise.abort(reason);
		});
	...
	* @return {AbortablePromise}
	*/
	downloadSplatSceneToSplatBuffer(path, splatAlphaRemovalThreshold = 1, onProgress = void 0, progressiveBuild = false, onSectionBuilt = void 0, format, headers) {
		try {
			if (format === SceneFormat.Splat || format === SceneFormat.KSplat || format === SceneFormat.Ply) {
				...
				if (format === SceneFormat.Splat) return SplatLoader.loadFromURL(path, onProgress, progressiveBuild, onSectionBuilt, splatAlphaRemovalThreshold, this.inMemoryCompressionLevel, optimizeSplatData, headers);
				else if (format === SceneFormat.KSplat) return KSplatLoader.loadFromURL(path, onProgress, progressiveBuild, onSectionBuilt, headers);
				else if (format === SceneFormat.Ply) return PlyLoader.loadFromURL(path, onProgress, progressiveBuild, onSectionBuilt, splatAlphaRemovalThreshold, this.inMemoryCompressionLevel, optimizeSplatData, this.sphericalHarmonicsDegree, headers);
			} else if (format === SceneFormat.Spz) return SpzLoader.loadFromURL(path, onProgress, splatAlphaRemovalThreshold, this.inMemoryCompressionLevel, this.optimizeSplatData, this.sphericalHarmonicsDegree, headers);
		} catch (e) {
	...
	*/
	addSplatBuffers = function() {
		return function(splatBuffers, splatBufferOptions = [], finalBuild = true, showLoadingUI = true, showLoadingUIForSplatTreeBuild = true, replaceExisting = false, enableRenderBeforeFirstSort = false, preserveVisibleRegion = true) {
			...
			this.splatRenderReady = false;
			return new Promise((resolve) => {
				if (showLoadingUI) splatProcessingTaskId = this.loadingSpinner.addTask("Processing splats...");
				delayedExecute(() => {
					if (this.isDisposingOrDisposed()) resolve();
					else {
						const buildResults = this.addSplatBuffersToMesh(splatBuffers, splatBufferOptions, finalBuild, showLoadingUIForSplatTreeBuild, replaceExisting, preserveVisibleRegion);
						const maxSplatCount = this.splatMesh.getMaxSplatCount();
						if (this.sortWorker && this.sortWorker.maxSplatCount !== maxSplatCount) this.disposeSortWorker();
						if (!this.gpuAcceleratedSort) this.preSortMessages.push({
						...
						(!this.sortWorker && maxSplatCount > 0 ? this.setupSortWorker(this.splatMesh) : Promise.resolve()).then(() => {
							...
							this.runSplatSort(true, true).then((sortRunning) => {
								if (!this.sortWorker || !sortRunning) {
									...
									if (enableRenderBeforeFirstSort) this.splatRenderReady = true;
									else this.runAfterNextSort.push(() => {
										this.splatRenderReady = true;
									});
	...
	*/
	addSplatBuffersToMesh = function() {
		...
		return function(splatBuffers, splatBufferOptions, finalBuild = true, showLoadingUIForSplatTreeBuild = false, replaceExisting = false, preserveVisibleRegion = true) {
			...
			const buildResults = this.splatMesh.build(allSplatBuffers, allSplatBufferOptions, true, finalBuild, onSplatTreeIndexesUpload, onSplatTreeReady, preserveVisibleRegion);
	...
	*/
	setupSortWorker(splatMesh) {
		...
		return new Promise((resolve) => {
			...
			this.sortWorker = createSortWorker(maxSplatCount, this.sharedMemoryForWorkers, this.enableSIMDInSort, this.integerBasedSort, this.splatMesh.dynamicMode, this.splatSortDistanceMapPrecision);
			this.sortWorker.onmessage = (e) => {
				if (e.data.sortDone) {
					this.sortRunning = false;
					if (this.sharedMemoryForWorkers) this.splatMesh.updateRenderIndexes(this.sortWorkerSortedIndexes, e.data.splatRenderCount);
					else {
				...
				} else if (e.data.sortCanceled) this.sortRunning = false;
					...
					if (this.sharedMemoryForWorkers) {
						...
						this.sortWorkerTransforms = new Float32Array(e.data.transformsBuffer, e.data.transformsOffset, Constants.MaxScenes * 16);
					} else {
	...
	updateError(error, defaultMessage) {
		...
		if (error instanceof DirectLoadError) return /* @__PURE__ */ new Error("File type or server does not support progressive loading.");
	...
	removeSplatScenes(indexesToRemove, showLoadingUI = true) {
		if (this.isLoadingOrUnloading()) throw new Error("Cannot remove splat scene while another load or unload is already in progress.");
		if (this.isDisposingOrDisposed()) throw new Error("Cannot remove splat scene after dispose() is called.");
		let sortPromise;
		this.splatSceneRemovalPromise = new Promise((resolve, reject) => {
			...
			sortPromise.then(() => {
				...
				for (let i = 0; i < this.splatMesh.scenes.length; i++) {
					...
					for (let indexToRemove of indexesToRemove) if (indexToRemove === i) {
						shouldRemove = true;
						break;
					}
					if (!shouldRemove) {
	...
	*/
	start() {
		if (this.selfDrivenMode) {
			if (this.webXRMode) this.renderer.setAnimationLoop(this.selfDrivenUpdateFunc);
	...
	*/
	stop() {
		if (this.selfDrivenMode && this.selfDrivenModeRunning) {
			if (this.webXRMode) this.renderer.setAnimationLoop(null);
			else cancelAnimationFrame(this.requestFrameId);
			this.selfDrivenModeRunning = false;
		}
	}
	/**
	...
	*/
	async dispose() {
		...
		let waitPromises = [];
		let promisesToAbort = [];
		for (let promiseKey in this.splatSceneDownloadPromises) if (this.splatSceneDownloadPromises.hasOwnProperty(promiseKey)) {
		...
		if (this.sortPromise) waitPromises.push(this.sortPromise);
		this.disposing = true;
		this.disposePromise = Promise.all(waitPromises).finally(() => {
			...
			if (!this.usingExternalRenderer) document.body.removeChild(this.rootElement);
			this.sortWorkerSortedIndexes = null;
			this.sortWorkerIndexesToSort = null;
			this.sortWorkerPrecomputedDistances = null;
			this.sortWorkerTransforms = null;
	...
	selfDrivenUpdate() {
		if (this.selfDrivenMode && !this.webXRMode) this.requestFrameId = requestAnimationFrame(this.selfDrivenUpdateFunc);
		this.update();
		if (this.shouldRender()) {
			this.render();
			this.consecutiveRenderFrames++;
		} else this.consecutiveRenderFrames = 0;
	...
	render = function() {
		return function() {
			...
			const hasRenderables = (threeScene) => {
				for (let child of threeScene.children) if (child.visible) return true;
				return false;
			};
	...
	update(renderer, camera) {
		...
		if (this.controls) {
			this.controls.update();
			if (this.camera.isOrthographicCamera && !this.usingExternalCamera) Viewer.setCameraPositionFromZoom(this.camera, this.camera, this.controls);
		}
		this.runSplatSort();
		...
		return function() {
			if (!this.usingExternalCamera) {
				...
				if (lastCameraOrthographic === void 0 || lastCameraOrthographic !== this.camera.isOrthographicCamera || currentRendererSize.x !== lastRendererSize.x || currentRendererSize.y !== lastRendererSize.y) {
					...
					} else this.camera.aspect = currentRendererSize.x / currentRendererSize.y;
	...
	updateCameraTransition = function() {
		...
		return function(currentTime) {
			if (this.transitioningCameraTarget) {
				...
				const t = (rotationAngle / (Math.PI / 3) * .65 + .3) / rotationAngle * (currentTime - this.transitioningCameraTargetStartTime);
				tempCameraTarget.copy(this.previousCameraTarget).lerp(this.nextCameraTarget, t);
				this.camera.lookAt(tempCameraTarget);
				this.controls.target.copy(tempCameraTarget);
				if (t >= 1) this.transitioningCameraTarget = false;
			}
		};
	}();
	updateFocusMarker = function() {
	...
	updateMeshCursor = function() {
		...
		return function() {
			if (this.showMeshCursor) {
				...
				} else this.sceneHelper.setMeshCursorVisibility(false);
			} else {
	...
	updateInfoPanel = function() {
		...
		return function() {
			...
			this.infoPanel.update(renderDimensions, this.camera.position, cameraLookAtPosition, this.camera.up, this.camera.isOrthographicCamera, meshCursorPosition, this.currentFPS || "N/A", splatCount, this.splatRenderCount, splatRenderCountPct, this.lastSortTime, this.focalAdjustment, this.splatMesh.getSplatScale(), this.splatMesh.getPointCloudModeEnabled());
		};
	}();
	updateControlPlane() {
		...
		} else this.sceneHelper.setControlPlaneVisibility(false);
	}
		...
		return function(force = false, forceSortAll = false) {
			...
			if (!force) {
				if (!this.splatMesh.dynamicMode && queuedSorts.length === 0) {
					if (angleDiff <= .99) needsRefreshForRotation = true;
			...
			if (this.gpuAcceleratedSort && (queuedSorts.length <= 1 || queuedSorts.length % 2 === 0)) gpuAcceleratedSortPromise = this.splatMesh.computeDistancesOnGPU(mvpMatrix, this.sortWorkerPrecomputedDistances);
			gpuAcceleratedSortPromise.then(() => {
				if (queuedSorts.length === 0) {
					if (this.splatMesh.dynamicMode || shouldSortAll) queuedSorts.push(this.splatRenderCount);
					else {
						for (let partialSort of partialSorts) if (angleDiff < partialSort.angleThreshold) {
							for (let sortFraction of partialSort.sortFractions) queuedSorts.push(Math.floor(this.splatRenderCount * sortFraction));
							break;
						}
				...
				if (this.splatMesh.dynamicMode) this.splatMesh.fillTransformsArray(this.sortWorkerTransforms);
				if (!this.sharedMemoryForWorkers) {
					sortMessage.indexesToSort = this.sortWorkerIndexesToSort;
					sortMessage.transforms = this.sortWorkerTransforms;
					if (this.gpuAcceleratedSort) sortMessage.precomputedDistances = this.sortWorkerPrecomputedDistances;
				}
				...
				this.sortWorker.postMessage({ "sort": sortMessage });
				if (queuedSorts.length === 0) {
			...
			if (splatTree) {
				...
				for (let s = 0; s < splatTree.subTrees.length; s++) {
					...
					for (let i = 0; i < nodeCount; i++) {
						...
						if (!gatherAllNodes && (outOfFovX || outOfFovY) && distanceToNode > ns) continue;
				...
				for (let i = 0; i < nodeRenderCount; i++) {
					...
					new Uint32Array(this.sortWorkerIndexesToSort.buffer, currentByteOffset - windowSizeBytes, windowSizeInts).set(node.data.indexes);
				...
				if (!allSplatsSortBuffer || allSplatsSortBuffer.length !== totalSplatCount) {
					...
					for (let i = 0; i < totalSplatCount; i++) allSplatsSortBuffer[i] = i;
				}
...
*/
var DropInViewer = class DropInViewer extends Group {
	constructor(options = {}) {
		super();
	...
	updateSplatMesh() {
		if (this.splatMesh !== this.viewer.splatMesh) {
			if (this.splatMesh) this.remove(this.splatMesh);
